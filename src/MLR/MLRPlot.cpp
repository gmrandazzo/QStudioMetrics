...
void MLRPlot::RecalcVSExperimentalAndPrediction(ScatterPlot **plot2D) {
  ...
  int did = projects->value(pid)->getMatrixID(
      projects->value(pid)->getMLRModel(mid)->getDataHash());
...
void MLRPlot::PredictedVSExperimentalAndPrediction(ScatterPlot **plot2D) {
  ...
  if (did > -1 && predid > -1) {
    ...
    if (pred_id > -1) {
      ...
      MatrixCopy(projects->value(pid)->getMLRModel(mid)->Model()->predicted_y,
                 &model_predicted_y);
...
void MLRPlot::RecalcVSExperimental(ScatterPlot **plot2D) {
  ...
  MatrixCopy(projects->value(pid)->getMLRModel(mid)->Model()->recalculated_y,
             &model_recalc_y);
...
void MLRPlot::RecalcResidualsVSExperimental(ScatterPlot **plot2D) {
  ...
  if (did > -1) {
    ...
    MatrixCopy(
        projects->value(pid)->getMLRModel(mid)->Model()->recalc_residuals,
        &recalc_res);
...
void MLRPlot::PredictedResidualsVSExperimental(ScatterPlot **plot2D) {
  ...
  if (did > -1) {
    ...
    matrix *pred_res;
    initMatrix(&pred_res);
    MatrixCopy(projects->value(pid)->getMLRModel(mid)->Model()->pred_residuals,
               &pred_res);
...
QList<ScatterPlot *> MLRPlot::YScramblingPlot() {
  ...
  size_t nobj =
      projects->value(pid)->getMLRModel(mid)->Model()->r2q2scrambling->row;
  size_t ndepvar =
      (projects->value(pid)->getMLRModel(mid)->Model()->r2q2scrambling->col -
       1) /
      2;
  ...
  for (size_t i = 0; i < nobj; i++) {
    mxlst[0]->data[i][0] = projects->value(pid)
                               ->getMLRModel(mid)
                               ->Model()
                               ->r2q2scrambling->data[i][0];
    mxlst[1]->data[i][0] = projects->value(pid)
                               ->getMLRModel(mid)
                               ->Model()
                               ->r2q2scrambling->data[i][0];
