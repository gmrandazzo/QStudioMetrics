...
{
  std::cout << "Usage:" << endl;
  std::cout << argv[0] << "\t -i <coordinate file>" << "\t -l <objnamelist>" << "\t -n <Number Of Compounds>" << "\t -c <id method>" << " \t -m <id metric space>" << std::endl;
  std::cout << "\t  Methods:\n 0: Random;\t1: MDC;\t2: MaxDissimilarity" << std::endl;
  std::cout << " \t Metrics:\n 0: Euclidean;\t1: Manhattan;\t2: Cosine"<< std::endl;
  ...
  if(argc <= 1){
    ...
    if(!matrixfile.empty() && !labelfile.empty()){
      ...
      for(uint i = 0; i < selections->size; i++){
        ...
        cout << getMatrixValue(m, getUIVectorValue(selections, i), m->col-1) << endl;
      }
