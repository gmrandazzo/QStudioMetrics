...
void PLSPlot::WeightsPlot2D(ScatterPlot **plot2D) {
  ...
  NewMatrix(&yloadingsbis,
            projects->value(pid)->getPLSModel(mid)->Model()->yloadings->row,
            projects->value(pid)->getPLSModel(mid)->Model()->yloadings->col);

  if (projects->value(pid)->getPLSModel(mid)->Model()->yloadings->row > 1) {
    for (uint j = 0;
         j < projects->value(pid)->getPLSModel(mid)->Model()->xweights->col;
         j++) {
      double xmin, xmax;
      double ymin, ymax;
      MatrixColumnMinMax(
          projects->value(pid)->getPLSModel(mid)->Model()->xweights, j, &xmin,
          &xmax);
      MatrixColumnMinMax(
          projects->value(pid)->getPLSModel(mid)->Model()->yloadings, j, &ymin,
          &ymax);
      ...
      */
      for (uint i = 0;
           i < projects->value(pid)->getPLSModel(mid)->Model()->yloadings->row;
           i++) {
        yloadingsbis->data[i][j] =
            (((xmax - xmin) * (projects->value(pid)
                                   ->getPLSModel(mid)
                                   ->Model()
                                   ->yloadings->data[i][j] -
                               ymin)) /
    ...
    for (uint j = 0;
      ...
      for (uint i = 0;
           i < projects->value(pid)->getPLSModel(mid)->Model()->yloadings->row;
           i++) {
        yloadingsbis->data[i][j] = xmax * projects->value(pid)
                                              ->getPLSModel(mid)
                                              ->Model()
                                              ->yloadings->data[i][j];
      }
...
void PLSPlot::ClassRecalcVSExperimental(QList<QStringList> *cellnames,
  ...
  if (projects->value(pid)->getPLSModel(mid)->getAlgorithm() == PLS_DA_) {
    ...
    for (size_t j = 0; j < nclass; j++) {
      ...
      NewMatrix(
          &mx,
          projects->value(pid)->getPLSModel(mid)->Model()->recalculated_y->row,
          3);
      for (size_t i = 0;
           i <
           projects->value(pid)->getPLSModel(mid)->Model()->recalculated_y->row;
        ...
        mx->data[i][0] =
            projects->value(pid)
                ->getPLSModel(mid)
                ->Model()
                ->recalculated_y->data[i][j + (nclass * nlv) - nclass];
      ...
      for (int i = 0; i < (int)mx->row; i++) {
        ...
        name +=
            QString("Score: %1").arg(QString::number(mx->data[i][0], 'g', 3));
        row_name << name;

        int pixid = -1;
        for (int k = 0; k < projects->value(pid)->getImages().size(); k++) {
          if (objsel[objid].compare(
                  projects->value(pid)->getImages()[k].name) == 0) {
            pixid = k;
            break;
          } else {
            continue;
...
void PLSPlot::ClassPredictedVSExperimental(QList<QStringList> *cellnames,
  ...
  if (projects->value(pid)->getPLSModel(mid)->getAlgorithm() == PLS_DA_) {
    ...
    for (size_t j = 0; j < nclass; j++) {
      ...
      NewMatrix(
          &mx,
          projects->value(pid)->getPLSModel(mid)->Model()->predicted_y->row, 3);
      for (size_t i = 0;
           i <
           projects->value(pid)->getPLSModel(mid)->Model()->predicted_y->row;
           i++) {
        mx->data[i][0] =
            projects->value(pid)
                ->getPLSModel(mid)
                ->Model()
                ->predicted_y->data[i][j + (nclass * nlv) - nclass];
        if (classes[j].objects.contains(objsel[i]) == true) {
          // True
          mx->data[i][1] = 1.f;
        } else {
...
QList<SimpleLine2DPlot *> PLSPlot::R2Q2() {
  ...
  if (projects->value(pid)->getPLSModel(mid)->getAlgorithm() == PLS_) {
    ...
    for (uint j = 0; j < yval; j++) {
      ...
      qDebug() << "r2";
      PrintMatrix(
          projects->value(pid)->getPLSModel(mid)->Model()->r2y_recalculated);
      qDebug() << "q2";
      ...
      for (uint i = 0; i < nlv - 1; i++) {
        setMatrixValue(m, i + 1, 1,
                       getMatrixValue(projects->value(pid)
                                          ...
                                          ->r2y_recalculated,
                                      i, l));
        double q2 = getMatrixValue(
            projects->value(pid)->getPLSModel(mid)->Model()->q2y, i, l);
        if (q2 < 0) {
  ...
  if (projects->value(pid)->getPLSModel(mid)->getAlgorithm() == PLS_) {
    ...
    for (uint j = 0; j < yval; j++) {
      ...
      for (uint i = 0; i < nlv; i++) {
        ...
        m->data[i][2] =
            projects->value(pid)->getPLSModel(mid)->Model()->sdep->data[i][l];
...
QList<SimpleLine2DPlot *> PLSPlot::ROCCurves() {
  ...
  if (projects->value(pid)->getPLSModel(mid)->getAlgorithm() == PLS_DA_) {
    ...
    tensor *roc_recalculated =
        projects->value(pid)->getPLSModel(mid)->Model()->roc_recalculated;
...
QList<SimpleLine2DPlot *> PLSPlot::PrecisionRecallCurves() {
  ...
  if (projects->value(pid)->getPLSModel(mid)->getAlgorithm() == PLS_DA_) {
    ...
    tensor *pr_recalculated = projects->value(pid)
                                  ->getPLSModel(mid)
                                  ->Model()
                                  ->precision_recall_recalculated;
    tensor *pr_predicted = projects->value(pid)
                               ->getPLSModel(mid)
                               ->Model()
...
QList<ScatterPlot *> PLSPlot::YScramblingPlot() {
  ...
  size_t nobj =
      projects->value(pid)->getPLSModel(mid)->Model()->yscrambling->row;
  size_t ndepvar =
      (projects->value(pid)->getPLSModel(mid)->Model()->yscrambling->col - 1) /
      2;
  ...
  for (size_t i = 0; i < nobj; i++) {
    mxlst[0]->data[i][0] =
        projects->value(pid)->getPLSModel(mid)->Model()->yscrambling->data[i]
                                                                          [0];
    mxlst[1]->data[i][0] =
        projects->value(pid)->getPLSModel(mid)->Model()->yscrambling->data[i]
                                                                          [0];
  ...
  for (size_t j = 0; j < ndepvar; j++) {
    for (size_t i = 0; i < nobj; i++) {
      mxlst[0]->data[i][1] = projects->value(pid)
                                 ->getPLSModel(mid)
                                 ->Model()
                                 ->yscrambling->data[i][j + 1];
      mxlst[1]->data[i][1] = projects->value(pid)
                                 ->getPLSModel(mid)
                                 ->Model()
                                 ->yscrambling->data[i][j + 1 + ndepvar];
    }
    plots2D.append(new ScatterPlot(mxlst, objnamelst,
                                   "Correlation with Y Real Vector", "R2/Q2",
                                   QString("PLS Y Scrambling Plot")));
