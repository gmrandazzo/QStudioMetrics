...
void UPLSPlot::TU_Plot(ScatterPlot2D **plot2D) {
  ...
  (*plot2D) = new ScatterPlot2D(
      mx, my, b, objname, &projects->value(pid)->getARRAYList(), xhash, yhash,
      &projects->value(pid)->getObjectLabels(),
      &projects->value(pid)->getVariableLabels(), QString("T"), QString("U"),
      QString("%1 | %2 | UPLS Score Plot (T/U)").arg(projectname).arg(modelname),
      ScatterPlot2D::SCORES);
  (*plot2D)->setPID(pid);
}
...
void UPLSPlot::T_ScorePlot2D(ScatterPlot2D **plot2D) {
  ...
  */
  (*plot2D) = new ScatterPlot2D(
      mxlst, objnamelst, "T", "T",
      QString("%1 | %2 | UPLS Score Plot (T/T)").arg(projectname).arg(modelname));
  (*plot2D)->setPID(pid);
}
...
void UPLSPlot::P_LoadingsPlot2D(QList<ScatterPlot2D *> *plot2D) {
  ...
  uint row =
      projects->value(pid)->getUPLSModel(mid)->Model()->xloadings->m[0]->row;
  uint col = projects->value(pid)->getUPLSModel(mid)->Model()->xloadings->order;
  uint layer =
      projects->value(pid)->getUPLSModel(mid)->Model()->xloadings->m[0]->col;
  ...
  for (uint k = 0; k < layer; k++) {
    ...
    for (uint i = 0; i < row; i++) {
      for (uint j = 0; j < col; j++) {
        setMatrixValue(
            m, i, j,
            getArrayValue(
                projects->value(pid)->getUPLSModel(mid)->Model()->xloadings, j,
                i, k));
      }
    }
    ...
    QList<QStringList> objnamelst;
    QStringList varname =
        projects->value(pid)->getUPLSModel(mid)->getXVarName();
    objnamelst.append(varname);
    QStringList xhash, yhash;
    xhash.append(projects->value(pid)->getUPLSModel(mid)->getXHash());
    yhash.append(projects->value(pid)->getUPLSModel(mid)->getYHash());
    (*plot2D).append(
        new ScatterPlot2D(mxlst, objnamelst, "P", "P",
                          QString("%1 | %2 | UPLS Loadings Plot (P) | Layer %3")
                              .arg(projectname)
                              .arg(modelname)
...
void UPLSPlot::WeightsPlot2D(QList<ScatterPlot2D *> *plot2D) {
  ...
  // generate a plot for each layer and the number of layer is  m[i]->col
  uint row =
      projects->value(pid)->getUPLSModel(mid)->Model()->xweights->m[0]->row;
  uint col = projects->value(pid)->getUPLSModel(mid)->Model()->xweights->order;
  uint layer =
      projects->value(pid)->getUPLSModel(mid)->Model()->xweights->m[0]->col;
  ...
  for (uint k = 0; k < layer; k++) {
    ...
    for (uint i = 0; i < row; i++) {
      for (uint j = 0; j < col; j++) {
        setMatrixValue(
            m, i, j,
            getArrayValue(
                projects->value(pid)->getUPLSModel(mid)->Model()->xweights, j,
                i, k));
      }
    }
    ...
    */
    (*plot2D).append(
        new ScatterPlot2D(mxlst, objnamelst, "W", "W",
                          QString("%1 | %2 | UPLS Weights Plot (W) | Layer %3")
                              .arg(projectname)
                              .arg(modelname)
                              .arg(QString::number(k + 1))));
    (*plot2D).last()->setPID(pid);
...
void UPLSPlot::U_ScorePlot2D(ScatterPlot2D **plot2D) {
  ...
  */
  (*plot2D) = new ScatterPlot2D(
      mxlst, objnamelst, "U", "U",
      QString("%1 | %2 | UPLS Score Plot (U/U)").arg(projectname).arg(modelname));
  (*plot2D)->setPID(pid);
}
...
void UPLSPlot::Q_LoadingsPlot2D(QList<ScatterPlot2D *> *plot2D) {
  ...
  // generate a plot for each layer and the number of layer is  m[i]->col
  uint row =
      projects->value(pid)->getUPLSModel(mid)->Model()->yloadings->m[0]->row;
  uint col = projects->value(pid)->getUPLSModel(mid)->Model()->yloadings->order;
  uint layer =
      projects->value(pid)->getUPLSModel(mid)->Model()->yloadings->m[0]->col;
  ...
  for (uint k = 0; k < layer; k++) {
    ...
    for (uint i = 0; i < row; i++) {
      for (uint j = 0; j < col; j++) {
        setMatrixValue(
            m, i, j,
            getArrayValue(
                projects->value(pid)->getUPLSModel(mid)->Model()->yloadings, j,
                i, k));
      }
    }
    ...
    */
    (*plot2D).append(
        new ScatterPlot2D(mxlst, objnamelst, "Q", "Q",
                          QString("%1 | %2 | UPLS Loadings Plot (Q) | Layer %3")
                              .arg(projectname)
                              .arg(modelname)
                              .arg(QString::number(k + 1))));
    (*plot2D).last()->setPID(pid);
...
void UPLSPlot::T_ScorePlotPrediction2D(ScatterPlot2D **plot2D) {
  ...
  */
  (*plot2D) =
      new ScatterPlot2D(mxlst, objnamelst, "T", "T",
                        QString("%1 | %2 | UPLS Score Plot (T/T) | Prediction")
                            .arg(projectname)
                            .arg(modelname));
  (*plot2D)->setPID(pid);
}

void UPLSPlot::RecalcVSExperimentalAndPrediction(
  ...
  // Get Prediction
  int ydid = projects->value(pid)->getArrayID(
      projects->value(pid)->getUPLSModel(mid)->getYHash());

  QStringList model_objsel =
      projects->value(pid)->getUPLSModel(mid)->getObjName();
  ...
  for (uint k = 0; k < projects->value(pid)->getArray(ydid)->Array()->order;
       k++) {
    ...
    for (uint i = 0; i < nobjects; i++) {
      for (uint j = 0; j < nvars; j++) {
        setMatrixValue(model_recalc_y, i, j,
                       getArrayValue(projects->value(pid)
                                         ->getUPLSModel(mid)
                                         ->Model()
                                         ->recalculated_y,
                                     k, i, j + (nvars * npc) - nvars));
      }
    }
    ...
    int ii = 0;
    for (int i = 0;
         i < projects->value(pid)->getArray(ydid)->getObjName().size(); i++) {
      if (model_objsel.contains(
              projects->value(pid)->getArray(ydid)->getObjName()[i]) == true) {
        int jy = 0;
        for (int j = 1;
             j < projects->value(pid)->getArray(ydid)->getVarName().size();
             j++) {
          if (yvarsel.contains(
                  projects->value(pid)->getArray(ydid)->getVarName()[j]) ==
              true) {
            setMatrixValue(
                model_exp_y, ii, jy,
                getArrayValue(projects->value(pid)->getArray(ydid)->Array(), k,
                              i, j - 1));
            jy++;
          } else {
            continue;
      ...
      for (uint j = 0; j < nvars; j++) {
        setMatrixValue(pred_predicted_y, i, j,
                       getArrayValue(projects->value(pid)
                                         ...
                                         ->getUPLSPrediction(predid)
                                         ->getYDipVar(),
                                     k + (nvars * npc) - nvars, i, j));
      }
    }

    int pred_id =
        projects->value(pid)->getArrayID(projects->value(pid)
                                             ->getUPLSModel(mid)
                                             ->getUPLSPrediction(predid)
                                             ->getYHash());
    matrix *pred_exp_y;
    ...
    ii = 0;
    for (int i = 0;
         i < projects->value(pid)->getArray(pred_id)->getObjName().size();
         i++) {
      if (pred_objsel.contains(
              projects->value(pid)->getArray(pred_id)->getObjName()[i]) ==
          true) {
        int jy = 0;
        for (int j = 1;
             j < projects->value(pid)->getArray(pred_id)->getVarName().size();
             j++) {
          if (yvarsel.contains(
                  projects->value(pid)->getArray(pred_id)->getVarName()[j]) ==
              true) {
            setMatrixValue(
                pred_exp_y, ii, jy,
                getArrayValue(projects->value(pid)->getArray(pred_id)->Array(),
                              k, i, j - 1));
            jy++;
          } else {
            continue;
    ...
    (*plot2D).append(new ScatterPlot2D(
        mx, my, objname, &projects->value(pid)->getMATRIXList(), xhash, yhash,
        &projects->value(pid)->getObjectLabels(),
        &projects->value(pid)->getVariableLabels(),
        QString("Layer %1 Experimental Y").arg(QString::number(k + 1)),
        QString("Layer %1 Recalculated and Predicted Y")
            .arg(QString::number(k + 1)),
        QString("%1 | %2 | UPLS Recalculated and Predicted VS Experimental Plot").arg(projectname).arg(modelname),
        ScatterPlot2D::SCORES));
...
void UPLSPlot::PredictedVSExperimentalAndPrediction(
    QList<ScatterPlot2D *> *plot2D) {
  QString projectname = projects->value(pid)->getProjectName();
  QString modelname = projects->value(pid)->getUPLSModel(mid)->getName();
  ...
  for (uint k = 0; k < projects->value(pid)->getArray(ydid)->Array()->order;
    ...
    for (uint i = 0; i < nobjects; i++) {
      for (uint j = 0; j < nvars; j++) {
        setMatrixValue(
            ...
            getArrayValue(
                projects->value(pid)->getUPLSModel(mid)->Model()->predicted_y,
                k, i, j + (nvars * npc) - nvars));
      }
    }
    ...
    (*plot2D).append(new ScatterPlot2D(
        ...
        &projects->value(pid)->getVariableLabels(),
        QString("Layer %1 Experimental Y").arg(QString::number(k + 1)),
        QString("Layer %1 Predicted Y").arg(QString::number(k + 1)),
        QString("%1 | %2 | UPLS Predicted VS Experimental Plot").arg(projectname).arg(modelname),
        ScatterPlot2D::SCORES));
    (*plot2D).last()->BuildDiagonal();
    (*plot2D).last()->setPID(pid);
    DelMatrix(&model_exp_y);
...
void UPLSPlot::RecalcVSExperimental(QList<ScatterPlot2D *> *plot2D) {
  ...
  for (uint k = 0; k < projects->value(pid)->getArray(ydid)->Array()->order;
    ...
    yhash.append(projects->value(pid)->getUPLSModel(mid)->getYHash());
    (*plot2D).append(new ScatterPlot2D(
        mx, my, objname, &projects->value(pid)->getARRAYList(), xhash, yhash,
        &projects->value(pid)->getObjectLabels(),
        &projects->value(pid)->getVariableLabels(),
        QString("Layer %1 Experimental Y").arg(QString::number(k + 1)),
        QString("Layer %1 Recalculated Y").arg(QString::number(k + 1)),
        QString("%1 | %2 | UPLS Recalculated VS Experimental Plot | Layer %3")
            .arg(projectname)
            .arg(modelname)
            .arg(QString::number(k + 1)),
        ScatterPlot2D::SCORES));
...
void UPLSPlot::RecalcResidualsVSExperimental(QList<ScatterPlot2D *> *plot2D) {
  ...
  for (uint k = 0; k < projects->value(pid)->getArray(ydid)->Array()->order;
    ...
    for (uint i = 0; i < nobjects; i++) {
      for (uint j = 0; j < nvars; j++) {
        setMatrixValue(model_recalc_res, i, j,
                       getArrayValue(projects->value(pid)
                                         ->getUPLSModel(mid)
                                         ->Model()
                                         ->recalc_residuals,
    ...
    (*plot2D).append(new ScatterPlot2D(
        ...
        QString("Layer %1 Experimental Y").arg(QString::number(k + 1)),
        QString("Layer %1 Recalculated Y").arg(QString::number(k + 1)),
        QString("%1 | %2 | UPLS Recalculated Residuals VS Experimental Plot | Layer %3")
            .arg(projectname)
            .arg(modelname)
            .arg(QString::number(k + 1)),
        ScatterPlot2D::SCORES));
...
void UPLSPlot::PredictedVSExperimental(QList<ScatterPlot2D *> *plot2D) {
  ...
  for (uint k = 0; k < projects->value(pid)->getArray(ydid)->Array()->order;
    ...
    (*plot2D).append(new ScatterPlot2D(
        ...
        QString("Layer %1 Experimental Y").arg(QString::number(k + 1)),
        QString("Layer %1 Predicted Y").arg(QString::number(k + 1)),
        QString("%1 | %2 | UPLS Predicted VS Experimental Plot | Layer %3")
            .arg(projectname)
            .arg(modelname)
            .arg(QString::number(k + 1)),
        ScatterPlot2D::SCORES));
...
void UPLSPlot::PredictedResidualsVSExperimental(
    QList<ScatterPlot2D *> *plot2D) {
  QString projectname = projects->value(pid)->getProjectName();
  QString modelname = projects->value(pid)->getUPLSModel(mid)->getName();
  ...
  for (uint k = 0; k < projects->value(pid)->getArray(ydid)->Array()->order;
    ...
    for (uint i = 0; i < nobjects; i++) {
      for (uint j = 0; j < nvars; j++) {
        setMatrixValue(model_pred_res, i, j,
                       getArrayValue(projects->value(pid)
                                         ->getUPLSModel(mid)
                                         ->Model()
                                         ->pred_residuals,
    ...
    (*plot2D).append(new ScatterPlot2D(
        ...
        &projects->value(pid)->getVariableLabels(),
        QString("Layer %1 Experimental Y").arg(QString::number(k + 1)),
        QString("Layer %1 Predicted Residuals Y").arg(QString::number(k + 1)),
        QString("%1 | %2 | UPLS Predicted Residuals VS Experimental Plot | Layer %3")
            .arg(projectname)
            .arg(modelname)
            .arg(QString::number(k + 1)),
        ScatterPlot2D::SCORES));
...
void UPLSPlot::R2Q2(QList<SimpleLine2DPlot *> *plot2D) {
  ...
  uint yval = projects->value(pid)
                  ...
                  ->r2y_model->m[0]
                  ->row;
  uint layers = projects->value(pid)
                    ->getUPLSModel(mid)
                    ->Model()
                    ->r2y_model->m[0]
                    ->col;
  ...
  if (projects->value(pid)->getUPLSModel(mid)->Model()->q2y_yscrambling->order >
      0) {
    NewMatrix(&m, model_npc + 1, 4);
    curvenames << "R2"
               << "Q2"
               << "Y Scrambling";
  } else {
    NewMatrix(&m, model_npc + 1, 3);
    curvenames << "R2"
               << "Q2";
  }
  ...
  for (uint j = 0; j < yval; j++) {
    for (uint k = 0; k < layers; k++) {
      ...
      if (projects->value(pid)
              ->getUPLSModel(mid)
              ->Model()
              ->q2y_yscrambling->order > 0) {
        columntitle << "Principal Components"
                    << QString("R^2 Y %1 Layer %2")
                           .arg(QString::number(j + 1))
                           .arg(QString::number(k + 1))
                    << QString("Q^2 Y %1 Layer %2")
                           .arg(QString::number(j + 1))
                           .arg(QString::number(k + 1))
                    << QString("Q^2 Model Consistency Y %1 Layer %2")
                           .arg(QString::number(j + 1))
                           .arg(QString::number(k + 1));
      } else {
        columntitle << "Principal Components"
      ...
      setMatrixValue(m, 0, 1, 0);
      setMatrixValue(m, 0, 2, 0);

      for (uint i = 0; i < model_npc; i++) {
        setMatrixValue(
            m, i + 1, 1,
            getArrayValue(
                projects->value(pid)->getUPLSModel(mid)->Model()->r2y_model, i,
                j, k));
        double q2 = getArrayValue(
            projects->value(pid)->getUPLSModel(mid)->Model()->q2y, i, j, k);
        if (q2 < 0) {
      ...
      (*plot2D).append(new SimpleLine2DPlot(
          m, curvenames,
          QString("%1 | %2 | R^2, Q^2 and Consistency Plot | Y %3 | Layer %4")
              .arg(projectname)
              .arg(modelname)
              .arg(j + 1)
              .arg(QString::number(k + 1)),
...
void UPLSPlot::R2R2Prediction(QList<SimpleLine2DPlot *> *plot2D) {
  ...
  for (uint j = 0; j < yval; j++) {
    for (uint k = 0; k < layers; k++) {
      ...
      if (getq2 == true) {
        columntitle << QString("Q^2 Y %1 Layer %2")
                           .arg(QString::number(j + 1))
                           .arg(QString::number(k + 1));
        curvenames << QString("Q^2 Y %1 Layer %2")
                          .arg(QString::number(j + 1))
                          .arg(QString::number(k + 1));
      }

      if (getq2consistency) {
        columntitle << QString("Y Scrambling Y %1 Layer %2")
                           .arg(QString::number(j + 1))
                           .arg(QString::number(k + 1));
        curvenames << QString("Y Scrambling Y %1 Layer %2")
                          .arg(QString::number(j + 1))
                          .arg(QString::number(k + 1));
      }

      columntitle << QString("R^2 Predicted Y %1 Layer %2")
                         .arg(QString::number(j + 1))
                         .arg(QString::number(k + 1));
      curvenames << QString("R^2 Predicted Y %1 Layer %2")
                        .arg(QString::number(j + 1))
                        .arg(QString::number(k + 1));

      int l = 1;
      setMatrixValue(m, 0, l, 0);
      l++;
      ...
      for (uint i = 0; i < (uint)npc; i++) {
        ...
        double r2pred = getArrayValue(projects->value(pid)
                                          ->getUPLSModel(mid)
                                          ->getUPLSPrediction(predid)
                                          ->getR2Y(),
                                      i, j, k);
      ...
      if (getq2) yaxisname += " / Q^2";
      if (getq2consistency) yaxisname += " / Consistency";
      yaxisname += " / R^2 Predicted";

      (*plot2D).append(
          new SimpleLine2DPlot(m, curvenames,
                               QString("%1 | %2 | Y %3 | R^2 / Predicted Plot")
                                   .arg(projectname)
                                   .arg(modelname)
                                   .arg(QString::number(j + 1)),
                               columntitle, "Principal Components", yaxisname));
      (*plot2D).last()->setLabelDetail(true);
    }
  }
  DelMatrix(&m);
...
void UPLSPlot::T_ScorePlot3D(ScatterPlot3D **plot3D) {
  ...
  (*plot3D) = new ScatterPlot3D(
      ...
      &projects->value(pid)->getObjectLabels(),
      &projects->value(pid)->getVariableLabels(),
      QString("%1 | %2 | UPLS Score Plot (T) (3D)").arg(projectname).arg(modelname),
...
void UPLSPlot::P_LoadingsPlot3D(QList<ScatterPlot3D *> *plot3D) {
  ...
  for (uint k = 0; k < layer; k++) {
    ...
    (*plot3D).append(new ScatterPlot3D(
        ...
        &projects->value(pid)->getObjectLabels(),
        &projects->value(pid)->getVariableLabels(),
        QString("%1 | %2 | UPLS Loadings Plot (P) (3D) | Layer %3")
            .arg(projectname)
            .arg(modelname)
            .arg(QString::number(k + 1)),
...
void UPLSPlot::WeightsPlot3D(QList<ScatterPlot3D *> *plot3D) {
  ...
  for (uint k = 0; k < layer; k++) {
    ...
    (*plot3D).append(new ScatterPlot3D(
        ...
        &projects->value(pid)->getObjectLabels(),
        &projects->value(pid)->getVariableLabels(),
        QString("%1 | %2 | UPLS Weights Plot (W) (3D) | Layer %3")
            .arg(projectname)
            .arg(modelname)
            .arg(QString::number(k + 1)),
        "PC", "PC", "PC", ScatterPlot3D::LOADINGS));
...
void UPLSPlot::U_ScorePlot3D(ScatterPlot3D **plot3D) {
  ...
  (*plot3D) = new ScatterPlot3D(
      ...
      &projects->value(pid)->getObjectLabels(),
      &projects->value(pid)->getVariableLabels(),
      QString("%1 | %2 | UPLS Score Plot (U) (3D)").arg(projectname).arg(modelname),
      "PC", "PC", "PC", ScatterPlot3D::SCORES);
  (*plot3D)->setPID(pid);
}
...
void UPLSPlot::Q_LoadingsPlot3D(QList<ScatterPlot3D *> *plot3D) {
  ...
  for (uint k = 0; k < layer; k++) {
    ...
    (*plot3D).append(new ScatterPlot3D(
        ...
        &projects->value(pid)->getObjectLabels(),
        &projects->value(pid)->getVariableLabels(),
        QString("%1 | %2 | UPLS Loadings Plot (Q) (3D) | Layer %3")
            .arg(projectname)
            .arg(modelname)
            .arg(QString::number(k + 1)),
        "PC", "PC", "PC", ScatterPlot3D::LOADINGS));
...
void UPLSPlot::T_ScorePlotPrediction3D(ScatterPlot3D **plot3D) {
  ...
  (*plot3D) =
      new ScatterPlot3D(mxlst, objname, &projects->value(pid)->getARRAYList(),
                        ...
                        &projects->value(pid)->getVariableLabels(),
                        QString("%1 | %2 | UPLS Score Plot Predictions (3D)")
                            .arg(projectname)
                            .arg(modelname),
                        "PC", "PC", "PC", ScatterPlot3D::SCORES);
  (*plot3D)->setPID(pid);
