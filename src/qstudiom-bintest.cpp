 ...
 *
 * Implementation file for qstudiom-bintest.
 *
...
void help(char **argv) {
  std::cout << "Usage:" << endl;
  std::cout << argv[0] << "\t -t <template coordinate file>"
            << "\t -a <active coordinate file>"
            << "\t -i <inactive coordinate file>"
            << "\t -d <decoys coordinate file>"
            << "\t-l <ligands coordinate files>"
            << "\t-o <output file>"
            << "\t-r  <resolution int>" << std::endl;
  std::cout
      << "\t  Methods:\n * -cd <int> if you want compute the distance. The "
         "number is for select between different Metrick: 1 = Euclidean "
      << std::endl;
  std::cout << " * -a <int> if you want autoscale the data: 1 for column SDEV; "
               "2 for column RMS"
            << std::endl;
  std::cout
      << "\n " << argv[0]
...
int main(int argc, char **argv) {
  if (argc <= 1) {
    ...
    for (int i = 0; i < argc; i++) {
      if (strcmp(argv[i], "-compute-distances") == 0 ||
          strcmp(argv[i], "-cd") == 0) {
        if (i + 1 < argc) {
    ...
    if (dtype != -1 && !template_.empty() && !active.empty() &&
        inactive.empty() && decoys.empty() &&
        !outputpath.empty()) { // Make only sensitivity by computing distances
      matrix *a, *t;
      matrix *dtp, *s;
      dvector *mm;
        ...
        if (a->col == t->col) {
          ...
          for (uint j = 0; j < a->col; j++) {
            if (FLOAT_EQ(getDVectorValue(colscaling, j), 0, 1 * 10e-8)) {
              ...
              for (uint k = 0; k < a->row; k++) {
                setMatrixValue(a, k, j,
                               getMatrixValue(a, k, j) /
                                   getDVectorValue(colscaling, j));
              }

              for (uint k = 0; k < t->row; k++) {
                setMatrixValue(t, k, j,
                               getMatrixValue(t, k, j) /
                                   getDVectorValue(colscaling, j));
              }
            }
          }
          ...
          fprintf(stderr,
                  "No Autoscaling Possible. The number of column differ.");
          fflush(stderr);
          abort();
        }
      }
      ...
      for (size_t k = 0; k < dtp->col; k++) {
        ...
        vector<string> strv;
        strv.push_back("#In order to calculate the real distance you need to "
                       "use this formula:");
        strv.push_back("#");
        strv.push_back(
            "#  Real_Distance = Normalized_Distance * (max-min) + min ");
      ...
      DelMatrix(&a);
      DelMatrix(&t);
    } else if (dtype == -1 && template_.empty() && !active.empty() &&
               inactive.empty() && decoys.empty() &&
               !outputpath
                    .empty()) { // Make only sensitivity by computing distances
      matrix *dtp, *s;
      dvector *mm;
      ...
      DelMatrix(&dtp);
               ...
               !inactive.empty() && decoys.empty() &&
               !outputpath.empty()) { // Make senitivity and PPV by computing
                                      // distances
      matrix *a, *i, *t;
        ...
        if (a->col == t->col) {
          ...
          for (uint k = 0; k < i->row; k++)
            for (uint j = 0; j < i->col; j++)
              setMatrixValue(mx, a->row + t->row + k, j,
                             getMatrixValue(i, k, j));
      ...
      if (dtp->col == dtn->col) {
        ...
        for (size_t k = 0; k < dtp->col; k++) {
          ...
          if (FLOAT_EQ(rmin, -99, 1 * 10e-8) ||
              FLOAT_EQ(rmax, -99, 1 * 10e-8)) {
            double t_min1, t_max1, t_min2, t_max2;
        ...
        fprintf(stderr,
                "Unable to do Sensitivity/Positive Predicted Value. There are "
                "different columns between True Positive and True Negative\n");
        fflush(stderr);
      }
      ...
      DelMatrix(&dtn);
               ...
               !inactive.empty() && !decoys.empty() &&
               !outputpath.empty()) { // Make senitivity, PPV and Abundance
        ...
        if (a->col == t->col) {
          ...
          for (uint k = 0; k < d->row; k++)
            for (uint j = 0; j < d->col; j++)
              setMatrixValue(mx, a->row + t->row + i->row + k, j,
                             getMatrixValue(d, k, j));
            ...
            if (FLOAT_EQ(getDVectorValue(colscaling, j), 0, 1 * 10e-8)) {
              ...
              if (!ligands.empty()) {
                for (uint k = 0; k < lig->row; k++) {
                  setMatrixValue(lig, k, j,
                                 getMatrixValue(lig, k, j) /
                                     getDVectorValue(colscaling, j));
                }
              }
            }
