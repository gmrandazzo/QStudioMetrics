...
void AdvancedPretreatmentDialog::OK()
{
 // Pretreat and Exit
  ...
  for(size_t j = 0; j < ncol; j++){
    ...
    // Set variable names in the pretreated matrix
    mx->getVarName().append(varnamestomod[j]);
    for(size_t i = 0; i < nrow; i++){
  ...
  if(ui.logTransform->isChecked()){
    Matrix2LogMatrix(mx_in, &mx->Matrix());
  }
  else if(ui.squareTransform->isChecked()){
    Matrix2SquareMatrix(mx_in, &mx->Matrix());
  }
  else if(ui.sqrtTransform->isChecked()){
    Matrix2SQRTMatrix(mx_in, &mx->Matrix());
  }
  else if(ui.absTransform->isChecked()){
  Matrix2ABSMatrix(mx_in, &mx->Matrix());
  }
  else if(ui.rowcenteringScaling->isChecked()){
    MatrixRowCenterScaling(mx_in, &mx->Matrix());
  }
  else{
    //else if(ui.SVNScaling->isChecked()){
    MatrixSVNScaling(mx_in, &mx->Matrix());
  }
...
void AdvancedPretreatmentDialog::OK()
{
 // Pretreat and Exit
  ...
  if(ui.addNoModVars->isChecked()){
    ...
    for(size_t j = 1; j < projects->value(pid)->getMatrix(mxid)->getVarName().size(); j++){
