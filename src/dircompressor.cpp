...
bool DirCompressor::SupportedFile(char *fname) {
  ...
  for (int i = 0; i < fsupported.size(); i++) {
    if (fname_.endsWith(fsupported[i].toLower()) == true ||
        fname_.endsWith(fsupported[i].toUpper()) == true) {
      return true;
    } else {
      continue;
    }
...
void DirCompressor::WriteFile(QStringList flist, char *path_) {
  ...
  if (flist.size() > 0 && path.exists()) {
    QString relfilepath =
        QString::fromUtf8(flist.first().split("@").last().toUtf8());

    /*qDebug() << QString::fromUtf8(relfilepath.toUtf8());*/
    QString fname = QString("%1/%2")
                        .arg(QString::fromUtf8(path.absolutePath().toUtf8()))
                        .arg(QString::fromUtf8(relfilepath.trimmed().toUtf8()));

    /*qDebug() << "WriteFile " << QString::fromUtf8(fname.toUtf8()) << " " <<
     * QString::fromUtf8(path_);*/

    QFileInfo finfo(QString::fromUtf8(fname.toUtf8()));
    if (!finfo.absoluteDir().exists()) {
      /*qDebug() << " Make Path... " <<
       * QString::fromUtf8(finfo.absolutePath().toUtf8());*/
...
void DirCompressor::ReadFileToString(char *fname, QStringList *filemem) {
  ...
  if (fname_.toLower().endsWith(".jpg") == true ||
...
#ifdef DEBUG
    qDebug() << "DirCompressor::ReadFileToString path: "
    ...
    /*
     QImage image;
      if(fname_.toLower().endsWith(".jpg") == true ||
     fname_.toLower().endsWith(".jpeg") == true) image.load(fname_, "JPG"); else
     if(fname_.toLower().endsWith(".png") == true) image.load(fname_, "PNG");
...
void DirCompressor::ScanDir(QDir dir, QFileInfoList *list) {
  QDirIterator iterator(QString::fromUtf8(dir.absolutePath().toUtf8()),
  ...
  while (iterator.hasNext()) {
    ...
    if (!iterator.fileInfo().isDir()) {
      ...
      if (SupportedFile(filename.toUtf8().data()) == true) {
        (*list).append(
            QFileInfo(QString::fromUtf8(iterator.filePath().toUtf8())));
      } else {
        continue;
      }
    }
...
//         printf("Compress %s\n",
filelist[i].absoluteFilePath().toUtf8().data()); QString filepath =
QString::fromUtf8(filelist[i].absoluteFilePath().toUtf8().data());
        ...
        ReadFileToString(filelist[i].absoluteFilePath().toUtf8().data(),
&dircontent);
      }
...
int DirCompressor::compress() {
  ...
  if (dirorfile.isEmpty() == false && out.isEmpty() == false) {
    ...
    if (dtype == DIRECTORY) {
      ...
      //       qCompress(codec->fromUnicode(dircontent.join("\n")));
      outfile.write(compressedData);
      outfile.close();
    } else {
      QFile infile(QString::fromUtf8(dirorfile.toUtf8()));
...
//   qDebug() << "Decompress";
//   printf("extract file %s to %s\n", dirorfile.toUtf8().data(),
extrpath.toUtf8().data());
  ...
  if(dirorfile.isEmpty() == false && f.isFile() && extrpath.isEmpty() == false){
    ...
    QString fname =
QString("%1/%2").arg(QString::fromUtf8(dir.absolutePath().toUtf8())).arg(tmp);
...
int DirCompressor::decompress() {
  ...
  //   qDebug() << f.isFile() << " " <<
  //   QString::fromUtf8(f.absoluteFilePath().toUtf8().data());

  if (dirorfile.isEmpty() == false && f.isFile() &&
      extrpath.isEmpty() == false) {
    QFile infile(QString::fromUtf8(dirorfile.toUtf8().data()));
    if (!infile.open(QIODevice::ReadOnly))
      return -1;
    ...
    GenRandomString(&tmp, 10);
    QString fname = QString("%1/%2")
                        .arg(QString::fromUtf8(dir.absolutePath().toUtf8()))
                        .arg(tmp);
    //     qDebug() << QString::fromUtf8(fname.toUtf8());
    QFile file(QString::fromUtf8(fname.toUtf8()));
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text))
...
void DirCompressor::setCompressionLevel(int clevel_) { clevel = clevel_; }
