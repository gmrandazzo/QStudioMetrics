...
QStringList ExportTableDialog::getSelectedObjects() {
  ...
  for (int i = 0; i < ui.listView_1->model()->rowCount(); i++) {
    if (ui.listView_1->selectionModel()->isSelected(
            ui.listView_1->model()->index(i, 0)) == true) {
      selectedobjects.append(
          ui.listView_1->model()->index(i, 0).data(Qt::DisplayRole).toString());
    } else {
      continue;
    }
  }
...
QStringList ExportTableDialog::getSelectedVariables() {
  ...
  for (int i = 0; i < ui.listView_2->model()->rowCount(); i++) {
    if (ui.listView_2->selectionModel()->isSelected(
            ui.listView_2->model()->index(i, 0)) == true) {
      selectedvariables.append(
          ui.listView_2->model()->index(i, 0).data(Qt::DisplayRole).toString());
    } else {
      continue;
    }
  }
...
void ExportTableDialog::SaveAs() {
  if (path.isEmpty()) {
    ui.filetosave->setText(
        QFileDialog::getSaveFileName(this, "Save Project", getenv("HOME")));
    QFileInfo last(ui.filetosave->text());
    path = QString::fromUtf8(last.absoluteFilePath().toUtf8());
  } else {
    ui.filetosave->setText(
        QFileDialog::getSaveFileName(this, "Save Project", path));
    QFileInfo last(ui.filetosave->text());
    path = QString::fromUtf8(last.absoluteFilePath().toUtf8());
  }
}
...
void ExportTableDialog::ObjSelectBy() {
  if (ui.objSelectByLabel->currentIndex() > 0) {
    ...
    for (int i = 0; i < m->rowCount(); i++) {
      if (objectlabels[labelindex].objects.contains(
              m->index(i, 0).data(Qt::DisplayRole).toString()) == true) {
    ...
    ui.listView_1->selectionModel()->select(selection,
...
void ExportTableDialog::VarSelectBy() {
  if (ui.varSelectByLabel->currentIndex() > 0) {
    ...
    QItemSelection selection;
    for (int i = 0; i < ui.listView_2->model()->rowCount(); i++) {
      if (varlabels[labelindex].objects.contains(ui.listView_2->model()
                                                     ->index(i, 0)
                                                     .data(Qt::DisplayRole)
                                                     .toString()) == true) {
    ...
    ui.listView_2->selectionModel()->select(selection,
                                            QItemSelectionModel::Select);
...
ExportTableDialog::ExportTableDialog(QStringList objname, QStringList varname,
  ...
  connect(ui.objunselectAllButton, SIGNAL(clicked(bool)),
          SLOT(ObjUnselectAll()));
  ...
  connect(ui.varunselectAllButton, SIGNAL(clicked(bool)),
          SLOT(VarUnselectAll()));
  connect(ui.varSelectByLabel, SIGNAL(currentIndexChanged(int)),
          SLOT(VarSelectBy()));
}
