  ...
  if (windowtype == MenuSelection) { /* no coordinate selection... */
    if (ui.listView->model()) {
      for (int i = 0; i < ui.listView->model()->rowCount(); i++) {
        if (ui.listView->selectionModel()->isSelected(
                ui.listView->model()->index(i, 0)) == true) {
          selectedvars.append(ui.listView->model()
                                  ->index(i, 0)
                                  .data(Qt::DisplayRole)
                                  .toString());
        } else {
...
int ObjectSelectorDialog::getDataID() { return dataindex; }
...
void ObjectSelectorDialog::SelectAll() {
  ...
  /*
  for(int i = 0; i < ui.listView->model()->rowCount(); i++){
    ui.listView->selectionModel()->select(ui.listView->model()->index(i, 0),
...
void ObjectSelectorDialog::SelectByVarLabel() {
  ...
  if (indx > 0) {
    for (int i = 0; i < ui.listView->model()->rowCount(); i++) {
      if ((*varlabels_)[indx - 1].objects.contains(ui.listView->model()
                                                       ->index(i, 0)
                                                       .data(Qt::DisplayRole)
                                                       .toString()) == true) {
        ui.listView->selectionModel()->select(ui.listView->model()->index(i, 0),
  ...
  if ((ui.mdcButton->isChecked() || ui.maxdissButton->isChecked())) {
    if (ui.plotcoordinatesButton->isChecked()) {
      ...
      if (!ui.listView->selectionModel()->selectedIndexes().isEmpty() &&
          ui.listView->selectionModel()->selectedIndexes().size() > 0) {
        return accept();
      } else {
        return;
      }
    ...
    return accept();
  } else if (ui.selectbyobjlabelbutton->isChecked() &&
             ui.objlabelslst->count() > 0) {
    return accept();
  } else {
    return;
  }
...
ObjectSelectorDialog::ObjectSelectorDialog(QList<MATRIX *> *mxlst,
  ...
  connect(ui.selectbyobjlabelbutton, SIGNAL(clicked(bool)),
          SLOT(EnableDisable()));
  ...
  connect(ui.listView_2->selectionModel(),
          ...
          SLOT(setVariablesView(QModelIndex)));
  connect(ui.selectAllButton, SIGNAL(clicked(bool)), SLOT(SelectAll()));
  connect(ui.invertSelectionButton, SIGNAL(clicked(bool)),
          SLOT(InvertSelection()));
  connect(ui.selectByVarLabel, SIGNAL(currentIndexChanged(int)),
          SLOT(SelectByVarLabel()));
...
ObjectSelectorDialog::ObjectSelectorDialog(QStringList &currentvariables,
                                           LABELS *objlabels, LABELS *varlabels)
    : QDialog() {
  ui.setupUi(this);
