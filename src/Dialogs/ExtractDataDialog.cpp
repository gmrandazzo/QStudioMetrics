...
void ExtractDataDialog::genObjVarView(QModelIndex current) {
  if (current.isValid()) {
    ...
    for (int i = 0;
         i <
         projects_->value(pid)->getMatrix(selectedhash_)->getObjName().size();
         i++) {
      //       for(int i = 0; i <
      //       (int)projects_->value(pid)->getMatrix(selectedhash_)->Matrix()->row;
      //       i++){
      QList<QStandardItem *> row;
      row.append(new QStandardItem(
          projects_->value(pid)->getMatrix(selectedhash_)->getObjName()[i]));
      tab3->appendRow(row);
    }

    for (int i = 0;
         i <
         projects_->value(pid)->getMatrix(selectedhash_)->getVarName().size();
         i++) {
      if (projects_->value(pid)
              ->getMatrix(selectedhash_)
              ->getVarName()[i]
              .compare(firstcol_name) == 0) {
        continue;
      } else {
        QList<QStandardItem *> row;
        row.append(new QStandardItem(
            projects_->value(pid)->getMatrix(selectedhash_)->getVarName()[i]));
        tab4->appendRow(row);
      }
    }
  ...
  if (pid != -1) {
    ...
    if (projects_->value(pid)->MatrixCount() > 0) {
      for (int i = 0; i < projects_->value(pid)->MatrixCount(); i++) {
        ...
        tab2matrix.append(
            new QStandardItem(projects_->value(pid)->getMatrix(i)->getName()));
...
void ExtractDataDialog::VariablesSelectBy() {
  if (pid != -1 && selectedhash_.size() > 0) {
    ...
    VariableSelectorDialog vseldialog(
        projects_->value(pid)->getMatrix(selectedhash_)->getVarName(),
    ...
    if (vseldialog.exec() == QDialog::Accepted) {
      ...
      for (int i = 0; i < ui.tableView_4->model()->rowCount(); i++) {
        if (varselected.indexOf(ui.tableView_4->model()
                                    ->index(i, 0)
                                    .data(Qt::DisplayRole)
                                    .toString()) > -1) {
    ...
    ui.tableView_4->selectionModel()->select(selection,
...
void ExtractDataDialog::VariablesSelectAll() {
  ...
  /*
  for(int i = 0; i < ui.tableView_4->model()->rowCount(); i++){
    ui.tableView_4->selectionModel()->select(ui.tableView_4->model()->index(i,
...
void ExtractDataDialog::ObjectsSelectBy() {
  ...
  if (doselectionfrommatrix == true) {
    if ((selectiontype == MOSTDESCRIPTIVECOMPOUND ||
        ...
        metric != -1 && nobjects != -1 && dataid != -1 && varlist.size() > 0) {
      matrix *m;
      uivector *selected;
      RUN obj;

      NewMatrix(&m, projects_->value(pid)->getMatrix(dataid)->Matrix()->row,
                varlist.size());
      QStringList varnames =
          projects_->value(pid)->getMatrix(dataid)->getVarName();
      ...
      for (int j = 0; j < varlist.size(); j++) {
        ...
        if (colindex != -1) {
          for (uint i = 0;
               i < projects_->value(pid)->getMatrix(dataid)->Matrix()->row;
               i++) {
            setMatrixValue(
                m, i, col,
                getMatrixValue(
                    projects_->value(pid)->getMatrix(dataid)->Matrix(), i,
                    colindex));
          }
          col++;
        } else {
      ...
      for (uint i = 0; i < selected->size; i++) {
        if (getUIVectorValue(selected, i) <
            projects_->value(pid)->getMatrix(dataid)->Matrix()->row) {
          ui.tableView_3->selectionModel()->select(
              ui.tableView_3->model()->index(getUIVectorValue(selected, i), 0),
      ...
      StopSelectionRun();
    } else if (selectiontype == RANDOMSELECTION && nobjects != -1 &&
               dataid != -1) {
      uivector *selected;
      RUN obj;
      ...
      if (label.size() > 0) {
        int lindex = -1;
        for (int i = 0; i < projects_->value(pid)->getObjectLabels().size();
             i++) {
          if (projects_->value(pid)->getObjectLabels()[i].name.compare(label) ==
              0) {
            lindex = i;
            break;
          } else {
        ...
        if (lindex > -1) {
          ...
          for (int i = 0;
               i <
               projects_->value(pid)->getObjectLabels()[lindex].objects.size();
               i++) {
            for (int j = 0; j < ui.tableView_3->model()->rowCount(); j++) {
              if (ui.tableView_3->model()
                      ...
                      .data(Qt::DisplayRole)
                      .toString()
                      .compare(projects_->value(pid)
                                   ->getObjectLabels()[lindex]
                                   .objects[i]) == 0) {
                ui.tableView_3->selectionModel()->select(
                    ui.tableView_3->model()->index(j, 0),
                    QItemSelectionModel::Select);
                break;
              } else {
                continue;
...
void ExtractDataDialog::ObjectsSelectAll() {
  ...
  for(int i = 0; i < ui.tableView_3->model()->rowCount(); i++){
    ui.tableView_3->selectionModel()->select(ui.tableView_3->model()->index(i,
  0), QItemSelectionModel::Select);
  }
  */
}
...
void ExtractDataDialog::UpdateSelectedObjectCounter() {
  ui.nselobj->setText(
      QString("%1/%2")
          .arg(QString::number(
              ui.tableView_3->selectionModel()->selectedIndexes().size()))
          .arg(QString::number(ui.tableView_3->model()->rowCount())));
}
...
void ExtractDataDialog::UpdateSelectedVariableCounter() {
  ui.nselvar->setText(
      QString("%1/%2")
          .arg(QString::number(
              ui.tableView_4->selectionModel()->selectedIndexes().size()))
          .arg(QString::number(ui.tableView_4->model()->rowCount())));
}

void ExtractDataDialog::OK() {
  QModelIndexList objectselected =
      ui.tableView_3->selectionModel()->selectedIndexes();
  QModelIndexList variableselected =
      ui.tableView_4->selectionModel()->selectedIndexes();

  if (objectselected.size() > 0 && variableselected.size() > 0 &&
    ...
    for (int i = 0; i < ui.tableView_3->model()->rowCount(); i++) {
      if (i == 0) { /*Fill the variable names*/
        for (int j = 0; j < ui.tableView_4->model()->rowCount(); j++) {
          if (ui.tableView_4->selectionModel()->isSelected(
                  ui.tableView_4->model()->index(j, 0)) == true) {
            varnames.append(projects_->value(pid)
                                ->getMatrix(selectedhash_)
                                ->getVarName()[j + 1]);
          } else {
            continue;
      ...
      if (ui.tableView_3->selectionModel()->isSelected(
              ui.tableView_3->model()->index(i, 0)) == true) {
        objnames.append(
            projects_->value(pid)->getMatrix(selectedhash_)->getObjName()[i]);
        col = 0;
        for (int j = 0; j < ui.tableView_4->model()->rowCount(); j++) {
          if (ui.tableView_4->selectionModel()->isSelected(
            ...
            setMatrixValue(
                _m_, row, col,
                getMatrixValue(
                    projects_->value(pid)->getMatrix(selectedhash_)->Matrix(),
                    i, j));
            col++;
          } else {
            continue;
    ...
    QMessageBox::warning(
        this, tr("Merge Data Error"),
        tr("Please select data and/or specify the new data name."),
        QMessageBox::Ok);
  }
}
...
ExtractDataDialog::ExtractDataDialog(PROJECTS *projects) : QDialog() {
  ...
  connect(ui.listView_2->selectionModel(),
          SIGNAL(currentChanged(QModelIndex, QModelIndex)),
          SLOT(genObjVarView(QModelIndex)));
  connect(ui.tableView_3->selectionModel(),
          SIGNAL(selectionChanged(QItemSelection, QItemSelection)),
          SLOT(UpdateSelectedObjectCounter()));
  connect(ui.tableView_4->selectionModel(),
          SIGNAL(selectionChanged(QItemSelection, QItemSelection)),
          SLOT(UpdateSelectedVariableCounter()));

  connect(ui.objSelectAllButton, SIGNAL(clicked(bool)),
          SLOT(ObjectsSelectAll()));
  connect(ui.objInvertSelectionButton, SIGNAL(clicked(bool)),
          SLOT(ObjectsInvertSelection()));
  ...
  connect(ui.varSelectAllButton, SIGNAL(clicked(bool)),
          SLOT(VariablesSelectAll()));
  connect(ui.varInvertSelectionButton, SIGNAL(clicked(bool)),
          SLOT(VariablesInvertSelection()));
  connect(ui.varSelectByButton, SIGNAL(clicked(bool)),
          SLOT(VariablesSelectBy()));
  connect(ui.varUnselectButton, SIGNAL(clicked(bool)),
          SLOT(VariablesUnselect()));
}
