...
void DoPredictionDialog::CheckDataForPrediction()
{
  if(selectedmodel_ > -1 && selecteddata_ > -1){
    ...
    else if(type == EPLS_ || type == EPLS_DA_){
      varsel = projects_->value(selectedproject_)->getEPLSModel(selectedmodel_)->getXVarName();
      vardata = projects_->value(selectedproject_)->getMatrix(selecteddata_)->getVarName();
    }
    else if(type == MLR_){
...
void DoPredictionDialog::VarInvertSelection()
{
  ...
  ui.listView_5->selectionModel()->select(selection, QItemSelectionModel::ClearAndSelect);
}

void DoPredictionDialog::VarSelectAll()
{
...
void DoPredictionDialog::ObjSelectBy()
{
  if(ui.objSelectByLabel->currentIndex() > 0){
    ...
    for(int i = 0; i < ui.listView_4->model()->rowCount(); i++){
      if(projects_->value(selectedproject_)->getObjectLabels()[labelindex].objects.contains(ui.listView_4->model()->index(i, 0).data(Qt::DisplayRole).toString()) == true){
    ...
    ui.listView_4->selectionModel()->select(selection, QItemSelectionModel::ClearAndSelect);
...
void DoPredictionDialog::ObjSelectAll()
{
...
void DoPredictionDialog::setProject(QModelIndex current)
{
  if(current.isValid()){
    ...
    else if(type == EPLS_ || type == EPLS_DA_){ // PLS
...
{
  if(state == 0 && selecteddata_ != -1 && selectedproject_ != -1 && !ui.predname->text().isEmpty()){
    state++;
    ...
    if(type == PCA_ || type == PLS_ || type == PLS_DA_ || type == EPLS_ || type == EPLS_DA_ || type == MLR_ || type == LDA_){
      ...
      if(type == PLS_ || type == MLR_){
        ...
        for(int i = 1; i < projects_->value(selectedproject_)->getMatrix(selecteddata_)->getVarName().size(); i++){
          ...
          vname.append(new QStandardItem(projects_->value(selectedproject_)->getMatrix(selecteddata_)->getVarName()[i]));
    ...
    QMessageBox::warning(this, tr("Warning!"), tr("No Project, data, model selected or model name specified!\nPlease select project and data and check the model name.\n"), QMessageBox::Close);
    return;
  }
}
  ...
  if(selectedproject_ == -1|| selectedmodel_ == -1 || selecteddata_ == -1 || ui.predname->text().isEmpty()){
    ...
    for(int i = 0; i < ui.listView_4->model()->rowCount(); i++){
      if(ui.listView_4->selectionModel()->isSelected(ui.listView_4->model()->index(i, 0)) == true){
        objsel.append(ui.listView_4->model()->index(i, 0).data(Qt::DisplayRole).toString());
      }
      else{
        continue;
      }
...
DoPredictionDialog::DoPredictionDialog(PROJECTS *projects, int type_)
{
  ...
  if(type == PCA_){
    ...
    setWindowTitle("Compute EPLS Prediction");
  }
  else if(type == MLR_){
 ...
 //Fill the table with data
  ...
  for(int i = 0; i < projects_->keys().size(); i++){
    ...
    projectsname.append(new QStandardItem(projects->value(pid)->getProjectName()));
    pids.append(pid);
  }
