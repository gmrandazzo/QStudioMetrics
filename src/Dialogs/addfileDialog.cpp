  ...
  while (!in.atEnd()) {
    ...
    if (line.contains("#", Qt::CaseInsensitive) == true) { // skip line
      continue;
               ...
               line.compare("-", Qt::CaseInsensitive) == true) {
      break;
    } else {
      nrow++;
    }
...
QStringList AddFileDialog::ListRead(QString file_) {
  ...
  while (!in.atEnd()) {
    lst.append(
        in.readLine().trimmed().split(getSeparator(), Qt::SkipEmptyParts));
  }
  file.close();
  return lst;
}
...
void AddFileDialog::setSimpleOpen() {
  ...
  ui.horizontalSpacer_3->changeSize(0, 0, QSizePolicy::Fixed,
                                    QSizePolicy::Fixed);
...
QString AddFileDialog::getSkipChar() { return ui.ignorelinebychar->text(); }
...
QString AddFileDialog::getLabel() {
  if (ui.filename->text().compare("project") !=
      0) // project name is a reserved name in order to recognize a speicif kind
         // of data in the qtreeview
    return ui.filename->text();
  else
    return ui.filename->text().append("_");
}

QString AddFileDialog::getFileName() { return ui.file->text(); }
...
void AddFileDialog::OpenFileObjName() {
  if (path.isEmpty()) {
    ui.file_objname->setText(QFileDialog::getOpenFileName(
        this, tr("Open File"), QDir::currentPath(),
        tr("Text Files (*.txt *.csv);;All files (*.*)"), 0,
        QFileDialog::DontUseNativeDialog));
...
void AddFileDialog::OpenFileVarName() {
  if (path.isEmpty()) {
    ui.file_varname->setText(QFileDialog::getOpenFileName(
        this, tr("Open File"), QDir::currentPath(),
        tr("Text Files (*.txt *.csv);;All files (*.*)"), 0,
        QFileDialog::DontUseNativeDialog));
...
void AddFileDialog::Preview() {
  if (!ui.file->text().isEmpty()) {
    ...
    while (!in.atEnd()) {
      ...
      if (line_.toStdString().find(getSkipChar().toStdString()) == 0 ||
          line_.isEmpty()) {
        continue;
            ...
            if (value.size() < 10) { // is the max number of column
              for (int c = 0; c < value.size(); c++) {
                model->setHorizontalHeaderItem(
                    c + 1, new QStandardItem(QString("Column %0").arg(c + 1)));
              }
            } else {
              for (int c = 0; c < 10; c++) {
                model->setHorizontalHeaderItem(
...
AddFileDialog::AddFileDialog() {
  ...
  connect(ui.buttonBox->button(QDialogButtonBox::Open), SIGNAL(clicked(bool)),
          this, SLOT(Open()));
  connect(ui.openbutton, SIGNAL(clicked(bool)), this, SLOT(OpenFileObjName()));
  connect(ui.openbutton_2, SIGNAL(clicked(bool)), this,
          SLOT(OpenFileVarName()));
...
AddFileDialog::~AddFileDialog() { delete model; }
