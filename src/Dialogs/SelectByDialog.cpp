...
void SelectByDialog::WaitSelectionRun() { ui.abortButton->setEnabled(false); }
...
void SelectByDialog::SetSelectionView() {
  ...
  if (ui.variableSelector->isChecked()) {
        ...
        if (minok == true && maxok == true) {
          for (int k = 0; k < mxid.size(); k++) {
            ...
            for (uint i = 0; i < (*mxlst)[id_]->Matrix()->row; i++) {
              ...
              if ((x > min || FLOAT_EQ(x, min, EPSILON)) &&
                  (x < max || FLOAT_EQ(x, max, EPSILON))) {
          ...
          QMessageBox::warning(
              this, tr("Select by matrix value"),
    ...
    if (selectiontype == MOSTDESCRIPTIVECOMPOUND ||
      ...
      if (metric != -1 && nobjects != -1 &&
          ((datatype == 1 && dataid != -1 && varlist.size() > 0) ||
           datatype == 0)) {
        matrix *m;
        uivector *selected;
        RUN obj;
          ...
          for (int j = 0; j < varlist.size(); j++) {
            ...
            if (colindex != -1) {
              for (uint i = 0; i < (*mxlst)[dataid]->Matrix()->row; i++) {
                setMatrixValue(
                    m, i, col,
                    getMatrixValue((*mxlst)[dataid]->Matrix(), i, colindex));
              }
              col++;
            } else {
        ...
        if (abort == false) {
          ...
          if (datatype == 1) {
            for (uint i = 0; i < selected->size; i++) {
              selection.append(
                  (*mxlst)[dataid]
                      ->getObjName()[getUIVectorValue(selected, i)]);
            }
            DelMatrix(&m);
          } else {
            for (uint i = 0; i < selected->size; i++) {
      ...
      if (nobjects != -1 &&
          ((datatype == 1 && dataid != -1) || datatype == 0)) {
        uivector *selected;
        RUN obj;
  ...
  if (append == true) {
    for (int i = 0; i < selection.size(); i++) {
      ...
      if (id != -1) {
        ui.objnamelistView->selectionModel()->select(
            ui.objnamelistView->model()->index(id, 0),
...
void SelectByDialog::UpdateSelectionCounter() {
  ui.selobjectlabel->setText(
      QString("%1/%2")
          .arg(QString::number(
              ui.objnamelistView->selectionModel()->selectedIndexes().size()))
          .arg(QString::number(ui.objnamelistView->model()->rowCount())));
}
...
void SelectByDialog::OpenSelectorButton() {
  ObjectSelectorDialog objseldialog(mxlst, objlabel, varlabel,
                                    ObjectSelectorDialog::PlotSelection);
...
void SelectByDialog::FindName() {
  ...
  if (!linetofind.isEmpty()) {
    for (int i = 0; i < ui.objnamelistView->model()->rowCount(); i++) {
      if (ui.objnamelistView->model()
              ->index(i, 0)
              .data(Qt::DisplayRole)
              .toString()
              .contains(linetofind) == true) {
        ui.objnamelistView->selectionModel()->select(
            ui.objnamelistView->model()->index(i, 0),
            QItemSelectionModel::Select);
...
void SelectByDialog::Abort() { abort = true; }
...
void SelectByDialog::OK() {
  for (int i = 0; i < ui.objnamelistView->model()->rowCount(); i++) {
    if (ui.objnamelistView->selectionModel()->isSelected(
            ui.objnamelistView->model()->index(i, 0)) == true) {
...
void SelectByDialog::Cancel() { reject(); }

SelectByDialog::SelectByDialog(QStringList objname_, matrix *coordinate_,
                               QList<MATRIX *> *mxlst_, QList<int> mxid_,
  ...
  connect(ui.objnamelistView->selectionModel(),
          SIGNAL(selectionChanged(QItemSelection, QItemSelection)),
          SLOT(UpdateSelectionCounter()));
  ...
  connect(ui.openSelectorButton, SIGNAL(clicked(bool)),
          SLOT(OpenSelectorButton()));
  connect(ui.selectButton, SIGNAL(clicked(bool)), SLOT(selectButton()));
  connect(ui.invertselectButton, SIGNAL(clicked(bool)),
          SLOT(invertselectButton()));
  connect(ui.appendselecttionButton, SIGNAL(clicked(bool)),
          SLOT(appendselectionButton()));
...
SelectByDialog::~SelectByDialog() { DelMatrix(&coordinate); }
