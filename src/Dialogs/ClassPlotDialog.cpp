...
void ClassPlotDialog::Find() {
  for (int i = 0; i < ui.listView_3->model()->rowCount(); i++) {
    if (ui.listView_3->model()
                ->index(i, 0)
                .data(Qt::DisplayRole)
                .toString()
                .contains(ui.lineEdit->text(), Qt::CaseSensitive) == true &&
        ui.listView_3->selectionModel()->isSelected(
            ui.listView_3->model()->index(i, 0)) == false) {
      ui.listView_3->scrollTo(ui.listView_3->model()->index(i, 0));
      ui.listView_3->selectionModel()->select(
          ui.listView_3->model()->index(i, 0), QItemSelectionModel::Select);
      break;
    } else {
      continue;
    }
  ...
  if (groupnamedialog.exec() == QDialog::Accepted) {
    ...
    if (selected.size() > 0) {
      ...
      for (int i = 0; i < selected.size(); i++) {
        //         QString name =
        //         ui.listView_3->selectionModel()->selectedRows()[i].data(Qt::DisplayRole).toString();
      ...
      while ((indexes = ui.listView_3->selectionModel()->selectedIndexes())
                 .size()) {
...
void ClassPlotDialog::SelectAll() { ui.listView_3->selectAll(); }
...
void ClassPlotDialog::SelectBy() {
  if (ui.selbycomboBox->currentIndex() - 1 < labels.size()) {
    ...
    for (int i = 0; i < ui.listView_3->model()->rowCount(); i++) {
      if (labels[i].objects.indexOf(ui.listView_3->model()
                                        ->index(i, 0)
                                        .data(Qt::DisplayRole)
                                        .toString()) > -1) {
...
#ifdef DEBUG
    qDebug() << "PLOTDIALOG SELECTED PROJECT: "
             << projects_->value(selectedproject_)->getProjectName()
             << " Model ID " << modelid;
#endif
    plot_ = true;
    accept();
...
ClassPlotDialog::ClassPlotDialog(PROJECTS *projects, int type_) {
  ...
  //   SIGNAL(currentChanged(QModelIndex,QModelIndex)), SLOT());
