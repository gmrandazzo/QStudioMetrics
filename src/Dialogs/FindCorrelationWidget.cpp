...
void FindCorrelationWidget::SelectAll() { ui.listView->selectAll(); }
...
void FindCorrelationWidget::PlotVarVSVar() {
  ...
  for(int i = 0; i < ui.listView->model()->rowCount(); i++){
    if(ui.listView->selectionModel()->isSelected(ui.listView->model()->index(i,
  0)) == true){ QStringList vars = ui.listView->model()->index(i,
  0).data(Qt::DisplayRole).toString().split(propstr); if(vars.size() == 2){
        vvplotSignal vvs;
  ...
  */
  ...
  for(int i = 0; i < selectionModel->selectedIndexes().size(); i++){
    QStringList vars =
  selectionModel->selectedIndexes()[i].data(Qt::DisplayRole).toString().split(propstr);
    if(vars.size() == 2){
...
void FindCorrelationWidget::Higlight() {
  ...
  QString propstr;
  propstr = QString::fromUtf8("\u221D");
  for(int i = 0; i < ui.listView->selectionModel()->selectedIndexes().size();
  i++){ QStringList vars =
  ui.listView->selectionModel()->selectedIndexes()[i].data(Qt::DisplayRole).toString().split(propstr);
    if(vars.size() == 2){
      highlightSignal hhs;
...
void FindCorrelationWidget::Find() {
  if (cmx->row > 0) {
    ...
    if (ui.allcorrelations->isChecked() == true) {
      for (uint i = 0; i < cmx->row; i++) {
        for (uint j = i + 1; j < cmx->col; j++) {
          if (FLOAT_EQ(cmx->data[i][j], 0, aerror) &&
              derr->data[i][j] < derror) {
            corr_0.append(CorrVarVarPlotSignal());
            corr_0.last().varid1 = i;
            corr_0.last().varid2 = j;
          } else if (FLOAT_EQ(cmx->data[i][j], 180, aerror) &&
                     derr->data[i][j] < derror) {
      ...
      if (ui.varlist->count() > 0) {
        QStringList selvarnames =
            varlabels->at(ui.varlist->currentIndex()).objects;
        ...
        for (int i = 0; i < selvars.size(); i++) {
          ...
          for (uint j = 0; j < cmx->col; j++) {
            if (selvars[i] != j) {
              if (FLOAT_EQ(cmx->data[selvars[i]][j], 0, aerror) &&
                  derr->data[selvars[i]][j] < derror) {
                corr_0.append(CorrVarVarPlotSignal());
                corr_0.last().varid1 = selvars[i];
                corr_0.last().varid2 = j;
              } else if (FLOAT_EQ(cmx->data[selvars[i]][j], 180, aerror) &&
                         derr->data[selvars[i]][j] < derror) {
                corr_180.append(CorrVarVarPlotSignal());
    ...
    if (ui.directly_proportional_checkBox->isChecked() == true &&
        ui.inversely_proportional_checkBox->isChecked() == true) {
      for (int i = 0; i < corr_0.size(); i++) {
        QList<QStandardItem *> row;
        row.append(new QStandardItem(QString("%1 %2 %3")
                                         .arg(varname[corr_0[i].varid1])
                                         .arg(propstr)
                                         .arg(varname[corr_0[i].varid2])));
        model->appendRow(row);
      }
      for (int i = 0; i < corr_180.size(); i++) {
        QList<QStandardItem *> row;
        row.append(new QStandardItem(QString("%1 %2 1/%3")
                                         .arg(varname[corr_180[i].varid1])
                                         .arg(propstr)
                                         .arg(varname[corr_180[i].varid2])));
        model->appendRow(row);
      }
    } else if (ui.directly_proportional_checkBox->isChecked() == true &&
               ui.inversely_proportional_checkBox->isChecked() == false) {
      for (int i = 0; i < corr_0.size(); i++) {
        QList<QStandardItem *> row;
        row.append(new QStandardItem(QString("%1 %2 %3")
                                         .arg(varname[corr_0[i].varid1])
    ...
    } else if (ui.directly_proportional_checkBox->isChecked() == false &&
               ui.inversely_proportional_checkBox->isChecked() == true) {
      for (int i = 0; i < corr_180.size(); i++) {
        QList<QStandardItem *> row;
        row.append(new QStandardItem(QString("%1 %2 1/%3")
...
FindCorrelationWidget::FindCorrelationWidget(matrix *cmx_, matrix *derr_,
  ...
  connect(ui.invertselectionButton, SIGNAL(clicked(bool)), this,
