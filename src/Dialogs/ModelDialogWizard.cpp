...
void ModelDialogWizard::BlockByLabel() {
  if (ui.xblock_SelectByLabel->currentIndex() > 0) {
    ...
    QStringList lstvars = projects_->value(selectedproject_)
                              ->getVariableLabels()[labelindex]
                              .objects;
    QString block_name = projects_->value(selectedproject_)
                             ->getVariableLabels()[labelindex]
                             .name;
...
void ModelDialogWizard::importBlock() {
  ...
  QString fileName = QFileDialog::getOpenFileName(
      this, tr("Open Variable Name List"), "", tr("Files (*.txt, *.csv)"));
...
void ModelDialogWizard::addBlock() {
  ...
  if (groupnamedialog.exec() == QDialog::Accepted) {
    ...
    if (selected.size() > 0) {
      ...
      while ((indexes = ui.listView_8->selectionModel()->selectedIndexes())
                 .size()) {
...
void ModelDialogWizard::ClassByLabel() {
  if (ui.class_SelectByLabel->currentIndex() > 0) {
    ...
    QStringList lstobjects = projects_->value(selectedproject_)
                                 ->getObjectLabels()[labelindex]
                                 .objects;
    QString class_name =
        projects_->value(selectedproject_)->getObjectLabels()[labelindex].name;
...
void ModelDialogWizard::importClass() {
  ...
  QString fileName = QFileDialog::getOpenFileName(
      this, tr("Open Object Name List"), "", tr("Files (*.txt, *.csv)"));
...
void ModelDialogWizard::addClass() {
  ...
  if (groupnamedialog.exec() == QDialog::Accepted) {
    ...
    if (selected.size() > 0) {
      ...
      // Remove selected index from the view
      QModelIndexList indexes;
      while ((indexes = ui.listView_6->selectionModel()->selectedIndexes())
                 .size()) {
    ...
    if (projects_->keys().contains(selectedproject_) == true) {
      if (projects_->value(selectedproject_)->MatrixCount() > 0) {
        for (int i = 0; i < projects_->value(selectedproject_)->MatrixCount();
          ...
          row_tab2.append(new QStandardItem(
      ...
      for (int i = 0;
        ...
        ui.class_SelectByLabel->addItem(
            projects_->value(selectedproject_)->getObjectLabels()[i].name);
      }
  ...
  if (ui.objSelectByLabel->currentIndex() > 0) {
    ...
    for (int i = 0; i < ui.listView_3->model()->rowCount(); i++) {
      if (projects_->value(selectedproject_)
              ->getObjectLabels()[labelindex]
              .objects.contains(ui.listView_3->model()
                                    ->index(i, 0)
                                    .data(Qt::DisplayRole)
                                    .toString()) == true) {
...
void ModelDialogWizard::XVarSelectBy() {
  if (ui.xvarSelectByLabel->currentIndex() > 0) {
    ...
    ui.listView_4->selectionModel()->select(selection,
                                            QItemSelectionModel::Select);
  ...
  if (selectedproject_ == -1 || selecteddata_ == -1 ||
    ...
    if (type == LDA_ || type == PLS_DA_ || type == EPLS_DA_) {
      for (int i = 0; i < ui.listView_3->model()->rowCount(); i++) {
        if (ui.listView_3->selectionModel()->isSelected(
          ...
          QString objname = ui.listView_3->model()
                                ->index(i, 0)
                                .data(Qt::DisplayRole)
                                .toString();
      ...
      for (int i = 0; i < ui.listView_3->model()->rowCount(); i++) {
        if (ui.listView_3->selectionModel()->isSelected(
                ui.listView_3->model()->index(i, 0)) == true) {
          objsel.append(ui.listView_3->model()
                            ->index(i, 0)
                            .data(Qt::DisplayRole)
                            .toString());
        } else {
    ...
    for (int i = 0; i < ui.listView_4->model()->rowCount(); i++) {
      if (ui.listView_4->selectionModel()->isSelected(
        ...
        xvarsel.append(ui.listView_4->model()
                           ->index(i, 0)
                           .data(Qt::DisplayRole)
                           .toString());
      } else {
      ...
      if (xvarsel.size() > 0) {
        if (type == PCA_) {
          ...
          if (yvarsel.size() > 0 && classes.size() == 0) {
            compute_ = true;
            accept();
          } else if (yvarsel.size() == 0 &&
                     classes.size() >= 2) { // at list a binary classification!
            compute_ = true;
            accept();
          } else {
            compute_ = false;
...
ModelDialogWizard::ModelDialogWizard(PROJECTS *projects, int type_,
  ...
  connect(ui.listView_1->selectionModel(),
          SIGNAL(currentChanged(QModelIndex, QModelIndex)),
          SLOT(genListView(QModelIndex, QModelIndex)));
  connect(ui.listView_2->selectionModel(),
          SIGNAL(currentChanged(QModelIndex, QModelIndex)),
          SLOT(setData(QModelIndex, QModelIndex)));
  ...
  connect(ui.xvarInvertSelectionButton, SIGNAL(clicked(bool)),
          SLOT(XVarInvertSelection()));
  connect(ui.xvarUnselectButton, SIGNAL(clicked(bool)),
          SLOT(XVarUnselectAll()));
  connect(ui.xvarSelectByLabel, SIGNAL(currentIndexChanged(int)),
          SLOT(XVarSelectBy()));
  ...
  connect(ui.yvarInvertSelectionButton, SIGNAL(clicked(bool)),
          SLOT(YVarInvertSelection()));
  connect(ui.yvarUnselectButton, SIGNAL(clicked(bool)),
          SLOT(YVarUnselectAll()));
  connect(ui.yvarSelectByLabel, SIGNAL(currentIndexChanged(int)),
          SLOT(YVarSelectBy()));

  connect(ui.modelname, SIGNAL(textChanged(QString)),
          SLOT(EnableDisableButtons()));
  connect(ui.listView_1->selectionModel(),
  ...
  connect(ui.listView_5->selectionModel(),
          SIGNAL(selectionChanged(QItemSelection, QItemSelection)),
          SLOT(EnableDisableButtons()));
  connect(ui.listView_6->selectionModel(),
          SIGNAL(selectionChanged(QItemSelection, QItemSelection)),
          SLOT(EnableDisableButtons()));
  connect(ui.listView_7->selectionModel(),
          SIGNAL(selectionChanged(QItemSelection, QItemSelection)),
          SLOT(EnableDisableButtons()));
  connect(ui.listView_8->selectionModel(),
          SIGNAL(selectionChanged(QItemSelection, QItemSelection)),
          SLOT(EnableDisableButtons()));
  connect(ui.listView_9->selectionModel(),
          SIGNAL(selectionChanged(QItemSelection, QItemSelection)),
          SLOT(EnableDisableButtons()));

  connect(ui.class_SelectByLabel, SIGNAL(currentIndexChanged(int)),
          SLOT(ClassByLabel()));
  ...
  connect(ui.xblock_SelectByLabel, SIGNAL(currentIndexChanged(int)),
          SLOT(BlockByLabel()));
  ...
  connect(ui.elmethodComboBox, SIGNAL(currentIndexChanged(int)),
          SLOT(ELmethodChanged(int)));

  connect(this->button(QWizard::NextButton), SIGNAL(clicked()), this,
          SLOT(next()));
  connect(this->button(QWizard::FinishButton), SIGNAL(clicked()), this,
          SLOT(OK()));
