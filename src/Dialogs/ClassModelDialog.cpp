...
void ClassModelDialog::next() {
  if (state == 0 && selecteddata_ > -1 &&
    ...
    for (int i = 0; i < projects_->value(selectedproject_)
         ...
         i++) {
      QList<QStandardItem *> mname;
      mname.append(new QStandardItem(projects_->value(selectedproject_)
                                         ->getMatrix(selecteddata_)
                                         ->getObjName()[i]));
    ...
    for (int i = 1; i < projects_->value(selectedproject_)
      ...
      name.append(new QStandardItem(projects_->value(selectedproject_)
                                        ->getMatrix(selecteddata_)
                                        ->getVarName()[i]));
      tab5->appendRow(name);
    }
  ...
  if (selectedproject_ == -1 || selecteddata_ == -1 ||
    ...
    for (int i = 0; i < ui.listView_5->model()->rowCount(); i++) {
      if (ui.listView_5->selectionModel()->isSelected(
        ...
        varsel.append(ui.listView_5->model()
                          ->index(i, 0)
                          .data(Qt::DisplayRole)
                          .toString());
      } else {
