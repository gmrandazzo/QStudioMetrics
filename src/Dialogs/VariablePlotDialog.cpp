...
void VariablePlotDialog::GenDataViewAndLabelView(QModelIndex current)
{
  if(current.isValid() && current.row() < projects->keys().size()){
    ...
    for(int i = 0; i < projects->value(pid)->MatrixCount(); i++){
      dataname.clear();
      dataname.append(new QStandardItem(projects->value(pid)->getMatrix(i)->getName()));
...
void VariablePlotDialog::GenVariableViewAndSetHash()
{
  ...
  if(pid > -1 && projects->keys().contains(pid) == true && indexes.size() > 0){
    ...
    Q_FOREACH(current, indexes){
      if(ui.dataView->model()->index(current.row(), 1).data(Qt::DisplayRole).toString().compare("Matrix") == 0){
    ...
    if(nmx > 0){// are matrix
	  ...
	  /*
          if(projects->value(pid)->getMatrix(hash[0])->getVarName()[i].compare(firstcol_name) == 0){
            continue;
          }
          else{*/
            ...
            if(tab5){
              ...
              varname2.clear();
              varname2.append(new QStandardItem(QString("%1").arg(projects->value(pid)->getMatrix(hash[0])->getVarName()[i])));
...
QStringList VariablePlotDialog::getObjLabelSelected()
{
  ...
  if(indexes.size() > 0){
    QModelIndex current;
    Q_FOREACH(current, indexes){
      labelobjsel.append(ui.objectsView->model()->data(current, Qt::DisplayRole).toString());
    }
  }
  return labelobjsel;
}
...
{
  return ui.variableView->model()->data(ui.variableView->selectionModel()->currentIndex(), Qt::DisplayRole).toString();
}

QString VariablePlotDialog::getVariableName2()
{
  return ui.variableView_2->model()->data(ui.variableView_2->selectionModel()->currentIndex(), Qt::DisplayRole).toString();
}
      ...
      if(current.row() == 0){ // append all objects
        ...
        if(hash.size() > 0){
          for(int i = 0; i < hash.size(); i++){
            for(int j = 0; j < projects->value(pid)->getMatrix(hash[i])->getObjName().size(); j++){
              obj.append(projects->value(pid)->getMatrix(hash[i])->getObjName()[j]);
            }
          }
        }
        else{
          for(int i = 0; i < hash.size(); i++){
            for(int j = 0; j < projects->value(pid)->getArray(hash[i])->getObjName().size(); j++){
              obj.append(projects->value(pid)->getArray(hash[i])->getObjName()[j]);
            }
          }
        }
        objects.append(obj);
        ...
        objects.append(projects->value(pid)->getObjectLabels()[current.row()-1].objects);
      }
    }
  }
  //ui.okButton->setEnabled(EnableOKButton());
...
void VariablePlotDialog::Cancel()
{
  reject();
}
    ...
    if(windowtype == VariableDistribution && varid1 > -1){
      ...
      for(int i = 0; i < hash.size(); i++){
        if(varid1 < (int)projects->value(pid)->getMatrix(hash[i])->Matrix()->col){
          ...
          QMessageBox::warning(this, tr("Warning!"), tr("Please the following matrix %1 present different variable types. Please select data with the same variables.\n"), QMessageBox::Close);
          return;
        }
      }
...
//     QMessageBox::warning(this, tr("Warning!"), tr("Please select a project.\n"), QMessageBox::Close);
    return;
  }
}
...
VariablePlotDialog::VariablePlotDialog(PROJECTS* projects_, int windowtype_): QDialog()
{
  ...
  if(windowtype == 0){
    ...
    connect(ui.variableView->selectionModel(), SIGNAL(currentChanged(QModelIndex,QModelIndex)), SLOT(setVariableID1(QModelIndex)));
    connect(ui.variableView_2->selectionModel(), SIGNAL(currentChanged(QModelIndex,QModelIndex)), SLOT(setVariableID2(QModelIndex)));
  }
  //ui.okButton->setEnabled(EnableOKButton());
}
