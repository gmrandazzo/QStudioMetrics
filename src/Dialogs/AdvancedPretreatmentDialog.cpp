...
void AdvancedPretreatmentDialog::OK() {
  ...
  for (int i = 0; i < ui.listView_3->selectionModel()->selectedRows().size();
       i++) {
    varnamestomod.append(ui.listView_3->selectionModel()
                             ->selectedRows()[i]
                             .data(Qt::DisplayRole)
                             .toString());
  }
  ...
  for (size_t j = 0; j < ncol; j++) {
    size_t colindx =
        projects->value(pid)->getMatrix(mxid)->getVarName().indexOf(
            varnamestomod[j]) -
        1;
    for (size_t i = 0; i < nrow; i++) {
      mx_in->data[i][j] =
          projects->value(pid)->getMatrix(mxid)->Matrix()->data[i][colindx];
    }
  }
  ...
  for (size_t i = 0; i < nrow; i++) {
    mx->getObjName().append(
        projects->value(pid)->getMatrix(mxid)->getObjName()[i]);
  }
  ...
  if (ui.addNoModVars->isChecked()) {
    mx->MatrixResize(
        projects->value(pid)->getMatrix(mxid)->getObjName().size(),
        projects->value(pid)->getMatrix(mxid)->getVarName().size() - 1);
    for (int j = 1;
         j < projects->value(pid)->getMatrix(mxid)->getVarName().size(); j++) {
      auto indx = varnamestomod.indexOf(
          projects->value(pid)->getMatrix(mxid)->getVarName()[j]);
      mx->getVarName().append(
          projects->value(pid)->getMatrix(mxid)->getVarName()[j]);
      if (indx > -1) {
        for (size_t i = 0; i < nrow; i++) {
        ...
        for (size_t i = 0; i < nrow; i++) {
          mx->Matrix()->data[i][j - 1] =
              projects->value(pid)->getMatrix(mxid)->Matrix()->data[i][j - 1];
        }
      }
    }
  } else {
...
void AdvancedPretreatmentDialog::setMatrixID(QModelIndex current) {
  if (current.isValid()) {
    ...
    if (mxid > -1) {
      tab3->clear();
      for (int i = 1;
           i < projects->value(pid)->getMatrix(mxid)->getVarName().size();
           i++) {
        QList<QStandardItem *> tab3row;
        tab3row.append(new QStandardItem(
            projects->value(pid)->getMatrix(mxid)->getVarName()[i]));
...
void AdvancedPretreatmentDialog::setProjectID(QModelIndex current) {
  if (current.isValid()) {
    ...
    for (int i = 0; i < projects->value(pid)->getVariableLabels().size(); i++) {
      ui.selectByLabelButton->addItem(
          projects->value(pid)->getVariableLabels()[i].name);
    }
...
void AdvancedPretreatmentDialog::EnableOKButton() {
  if (pid > -1 && mxid > -1 && !ui.dataname->text().isEmpty() &&
...
void AdvancedPretreatmentDialog::SelectVarsBy(int indx) {
  if (pid > -1 && mxid > -1) {
    ...
    for (int i = 0; i < ui.listView_3->model()->rowCount(); i++) {
      if (projects->value(pid)->getVariableLabels()[indx].objects.contains(
              ui.listView_3->model()
                  ->index(i, 0)
                  .data(Qt::DisplayRole)
                  .toString()) == true) {
...
AdvancedPretreatmentDialog::AdvancedPretreatmentDialog(PROJECTS *projects_) {
  ...
  connect(ui.invertSelectionButton, SIGNAL(clicked()),
          SLOT(InvertVarSelection()));
  connect(ui.unselectButton, SIGNAL(clicked()), SLOT(UnselectAllVars()));
  connect(ui.selectByLabelButton, SIGNAL(currentIndexChanged(int)),
          SLOT(SelectVarsBy(int)));
