...
void ModelDialog::genListView(QModelIndex current)
{
  if(current.isValid()){
    ...
    if(projects_->keys().contains(selectedproject_) == true){
      ...
      else if(type == UPCA_ || type == UPLS_){
        if(projects_->value(selectedproject_)->ArrayCount() > 0){
          for(int i = 0; i < projects_->value(selectedproject_)->ArrayCount(); i++){
            QList<QStandardItem*> tab2array, tab6array;
            tab2array.append(new QStandardItem(projects_->value(selectedproject_)->getArray(i)->getName()));
            tab6array.append(new QStandardItem(projects_->value(selectedproject_)->getArray(i)->getName()));
            tab2->appendRow(tab2array);
            tab6->appendRow(tab6array);
          }
        }
      }
...
void ModelDialog::next()
{
  if(type == PCA_){
    ...
    if(state == 0 && selecteddata_ > -1 &&  projects_->keys().contains(selectedproject_) == true && !ui.modelname->text().isEmpty()){
      ...
      QMessageBox::warning(this, tr("Warning!"), tr("No Project or data selected or UPCA model name specified!\nPlease select project and data and check the UPLS model name.\n"), QMessageBox::Close);
      return;
    }
  }
//  else if(type == UPLS_){
    ...
    if(state == 0 && selecteddata_ > -1 && ydata > -1 &&  projects_->keys().contains(selectedproject_) == true && !ui.modelname->text().isEmpty()){
      ...
      QMessageBox::warning(this, tr("Warning!"), tr("No Project or data selected or UPLS model name specified!\nPlease select project and data and check the UPLS model name.\n"), QMessageBox::Close);
      return;
    }
  }
}
...
ModelDialog::ModelDialog(PROJECTS *projects, int type_)
{
  ...
  if(type == PCA_){
    ...
    setWindowTitle("Compute UPLS");
    ui.dataGroupBox->setTitle("X Data Array");
    ui.dataGroupBox_2->setTitle("Y Data Array");
  }
 ...
 //Fill the table with data
  ...
  connect(ui.listView_6->selectionModel(), SIGNAL(currentChanged(QModelIndex,QModelIndex)), SLOT(setYData(QModelIndex)));
