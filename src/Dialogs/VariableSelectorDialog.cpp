...
QStringList VariableSelectorDialog::getSelectedVariables()
{
  ...
  for(int i = 0; i < ui.varnamelistView->model()->rowCount(); i++){
    if(ui.varnamelistView->selectionModel()->isSelected(ui.varnamelistView->model()->index(i, 0)) == true){
      varselected.append(ui.varnamelistView->model()->index(i, 0).data(Qt::DisplayRole).toString());
    }
    else{
      continue;
    }
...
if(ui.selectbyregexpButton->isChecked()){
    for(int i = 0; i < ui.varnamelistView->model()->rowCount(); i++){
      if(ui.varnamelistView->model()->index(i, 0).data(Qt::DisplayRole).toString().contains(ui.regexp->text(), Qt::CaseSensitive) == true){
    ...
    if(index > -1){
      for(int i = 0; i < ui.varnamelistView->model()->rowCount(); i++){
        ...
        for(int j = 0; j < (*varlabels)[index].objects.size(); j++){
          if(ui.varnamelistView->model()->index(i, 0).data(Qt::DisplayRole).toString().contains((*varlabels)[index].objects[j], Qt::CaseSensitive) == true){
            select = true;
            break;
          }
          else{
            ...
            if(var == -1){ // All variables
              for(int j = 0; j < (int)m[id[k]]->Matrix()->col; j++){
                if(objselectiontype == 0){
                  if(getMatrixValue(m[id[k]]->Matrix(), indx, j) < limitinf || getMatrixValue(m[id[k]]->Matrix(), indx, j) > limitsup){
                  ...
                  if(getMatrixValue(m[id[k]]->Matrix(), indx, j) > min && getMatrixValue(m[id[k]]->Matrix(), indx, j) < max){
                    selvar.append(m[id[k]]->getVarName()[j+1]);
                  }
                  else{
                    continue;
              ...
              if(objselectiontype == 0){
                if(getMatrixValue(m[id[k]]->Matrix(), indx, var) < limitinf|| getMatrixValue(m[id[k]]->Matrix(), indx, var) > limitsup){
                ...
                if(getMatrixValue(m[id[k]]->Matrix(), indx, var) > min && getMatrixValue(m[id[k]]->Matrix(), indx, var) < max){
                  selvar.append(m[id[k]]->getVarName()[var+1]);
                }
                else{
                  continue;
    ...
    for(int i = 0; i < ui.varnamelistView->model()->rowCount(); i++){
      if(selvar.indexOf(ui.varnamelistView->model()->index(i, 0).data(Qt::DisplayRole).toString()) > -1){
        QModelIndex topLeft = ui.varnamelistView->model()->index(i, 0);
        QModelIndex bottomRight = ui.varnamelistView->model()->index(i, 0);
        selection << QItemSelectionRange(topLeft, bottomRight);
      }
...
{
  ui.selectedlabel->setText(QString("%1 / %2").arg(ui.varnamelistView->selectionModel()->selectedRows().size()).arg(ui.varnamelistView->model()->rowCount()));
}
...
QStringList VariableSelectorDialog::getObjList()
{
  ...
  for(int i = 0; i < ui.objectlistView->model()->rowCount(); i++){
    if(ui.objectlistView->selectionModel()->isSelected(ui.objectlistView->model()->index(i, 0)) == true){
      selected.append(ui.objectlistView->model()->index(i, 0).data(Qt::DisplayRole).toString());
    }
    else{
      continue;
    }
...
{
  for(int i = 0; i < ui.varnamelistView->model()->rowCount(); i++){
    if(ui.varnamelistView->model()->index(i, 0).data(Qt::DisplayRole).toString().contains(ui.searchline->text(), Qt::CaseSensitive) == true
      && ui.varnamelistView->selectionModel()->isSelected(ui.varnamelistView->model()->index(i, 0)) == false){
      ui.varnamelistView->scrollTo(ui.varnamelistView->model()->index(i, 0));
      ui.varnamelistView->selectionModel()->select(ui.varnamelistView->model()->index(i, 0), QItemSelectionModel::Select);
      break;
    }
    else{
      continue;
...
void VariableSelectorDialog::Select()
{
  ...
  ui.varnamelistView->selectionModel()->select(selection, QItemSelectionModel::Select);
}
...
void VariableSelectorDialog::OK()
{
  accept();
}
...
VariableSelectorDialog::VariableSelectorDialog(QStringList &objnames, QStringList &variablenames, QList < MATRIX* > m_, QList< int > id_, LABELS *varlabels_)
{
  ui.setupUi(this);
  ...
  ui.label_13->hide(); ui.objlayerBox->hide();
  ...
  connect(ui.varnamelistView->selectionModel(), SIGNAL(selectionChanged(QItemSelection,QItemSelection)), SLOT(UpdateLabelSelection()));
  connect(ui.selectButton, SIGNAL(clicked(bool)), SLOT(Select()));
  connect(ui.invertselectButton, SIGNAL(clicked(bool)), SLOT(InvertSelection()));
  connect(ui.appendselectionButton, SIGNAL(clicked(bool)), SLOT(AppendSelection()));
  ...
  connect(ui.selectbyvarlabelButton, SIGNAL(clicked(bool)), SLOT(EnableDisable()));
  connect(ui.selectbyregexpButton, SIGNAL(clicked(bool)), SLOT(EnableDisable()));
  connect(ui.selectbyObj, SIGNAL(clicked(bool)), SLOT(EnableDisable()));
  EnableDisable();
  UpdateLabelSelection();
}

VariableSelectorDialog::VariableSelectorDialog(QStringList &variablenames, LABELS *varlabels_)
{
  ui.setupUi(this);
