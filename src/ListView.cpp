...
void ListView::copy()
{
  ...
  for(int i = 0; i < ui.listView->selectionModel()->selectedRows().size(); ++i)
    strings << ui.listView->selectionModel()->selectedRows()[i].data(Qt::DisplayRole).toString();
  QApplication::clipboard()->setText(strings.join("\n"));
}

void ListView::exportlst()
{
 QString fileName = QFileDialog::getSaveFileName(this, tr("Export list to file..."), "", tr("TXT (*.txt);;All Files (*)"));
