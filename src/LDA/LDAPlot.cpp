...
void LDAPlot::FeaturePlot2D(ScatterPlot **plot2D) {
  if (mid > -1) {
    ...
    for (uint k = 0;
      ...
      for (uint i = 0;
           i <
           projects->value(pid)->getLDAModel(mid)->Model()->features->m[k]->row;
           i++) {
        objnamelst.last().append(projects->value(pid)
                                     ->getLDAModel(mid)
...
void LDAPlot::ProbabilityDistribution(ScatterPlot **plot2D) {
  if (mid > -1) {
    ...
    for (uint l = 0; l < lda->evect->col; l++) { // for each ldfeature
      for (uint k = 0; k < lda->features->order;
           k++) { // for each class of objects...
        dvector *ldfeature;
        NewDVector(&ldfeature, lda->features->m[k]->row);
        for (uint i = 0; i < ldfeature->size; i++) {
          ldfeature->data[i] =
              lda->features->m[k]->data[i][l]; // copy the feature
        }
...
QList<SimpleLine2DPlot *> LDAPlot::ROCPlot() {
  ...
  if (mid > -1) {
    ...
    curvenames << "Current classifier"
               << "Random classifier";

    for (size_t k = 0; k < lda->roc->order; k++) {
      QString classname =
          projects->value(pid)->getLDAModel(mid)->getNameClasses()[k];
