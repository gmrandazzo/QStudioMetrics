...
{
  std::cout << "Usage:" << endl;
  std::cout << " Yates Effect: " << argv[0] << "-y <matrix coordinates>" <<"\t -o <output file>" << std::endl;
  std::cout << " RSM : " << argv[0] << "-rsm <matrix coordinates> <y coordinates>" <<"\t -o <output file>" << std::endl;
...
int main(int argc, char **argv)
{
  if(argc <= 1){
    ...
    else if(!x_.empty() && !y_.empty() && !outputfile.empty()){ //Make Response Surface Metholodogy
