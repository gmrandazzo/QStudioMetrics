...
public:
  ...
  void newMatrix(uint row, uint col, QStringList objnames,
...
private:
  ...
  int variable;
  QColor makeColor(double val, const double &min, const double &max,
...
public:
  ...
  explicit Table(matrix *m_, LABELS *objlabels_, LABELS *varlabels_,
                 QWidget *parent = 0);
  explicit Table(QList<QStringList> tab, LABELS *objlabels_, LABELS *varlabels_,
                 QWidget *parent = 0);
  explicit Table(QStringList names, QList<QPixmap> images, QList<QColor> colors,
                 QWidget *parent = 0);
  ~Table();
