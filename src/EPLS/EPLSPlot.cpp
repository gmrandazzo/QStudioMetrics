...
void EPLSPlot::PredictedVSExperimentalAndPrediction(ScatterPlot **plot2D) {
  ...
  int did = projects->value(pid)->getMatrixID(
      projects->value(pid)->getEPLSModel(mid)->getDataHash());
  if (did > -1 && predid > -1) {
    ...
    if (pred_id > -1) {
      ...
      auto nobjects = model_objsel.size();
    ...
    pos 0 1    2   3   4   5    6    7
      */
      ...
      for (uint i = 0; i < nobjects; i++) {
        ...
        for (uint j = 0; j < ny; j++) {
          setMatrixValue(
              ...
              getMatrixValue(
                  projects->value(pid)->getEPLSModel(mid)->y_predicted, i,
                  j + (ny * nlv) - ny));
        }
      }
      ...
      QStringList pred_objsel = projects->value(pid)
                                    ...
                                    ->getEPLSPrediction(predid)
                                    ->getObjName();
      auto pred_nobjects = pred_objsel.size();
...
void EPLSPlot::ClassPredictedVSExperimental(QList<QStringList> *cellnames,
  ...
  for (size_t j = 0; j < nclass; j++) {
    ...
    NewMatrix(&mx, projects->value(pid)->getEPLSModel(mid)->y_predicted->row,
              3);
    for (size_t i = 0;
         i < projects->value(pid)->getEPLSModel(mid)->y_predicted->row; i++) {
      mx->data[i][0] = projects->value(pid)
                           ->getEPLSModel(mid)
                           ->y_predicted->data[i][j + (nclass * nlv) - nclass];
    ...
    for (int i = 0; i < (int)mx->row; i++) {
      ...
      for (int k = 0; k < projects->value(pid)->getImages().size(); k++) {
        if (objsel[objid].compare(projects->value(pid)->getImages()[k].name) ==
            0) {
          pixid = k;
          break;
        } else {
...
void EPLSPlot::RecalcVSExperimental(ScatterPlot **plot2D) {
  ...
  */
  ...
  for (uint i = 0; i < nobjects; i++) {
    ...
    for (uint j = 0; j < ny; j++) {
      setMatrixValue(
          model_recalc_y, i, j,
          getMatrixValue(
              projects->value(pid)->getEPLSModel(mid)->y_recalculated, i,
              j + (ny * nlv) - ny));
    }
  }
  ...
  NewMatrix(&model_exp_y, nobjects, nvars);
  int ii = 0;
  for (int i = 0; i < projects->value(pid)->getMatrix(did)->getObjName().size();
       i++) {
    if (objsel.contains(
            projects->value(pid)->getMatrix(did)->getObjName()[i]) == true) {
      int jy = 0;
      for (int j = 1;
           j < projects->value(pid)->getMatrix(did)->getVarName().size(); j++) {
        if (yvarsel.contains(
                projects->value(pid)->getMatrix(did)->getVarName()[j]) ==
            true) {
          setMatrixValue(
              model_exp_y, ii, jy,
              getMatrixValue(projects->value(pid)->getMatrix(did)->Matrix(), i,
                             j - 1));
          jy++;
        } else {
          continue;
...
void EPLSPlot::ClassRecalcVSExperimental(QList<QStringList> *cellnames,
                                         QList<QList<QPixmap>> *images,
                                         QList<QList<QColor>> *colors) {
  QString projectname = projects->value(pid)->getProjectName();
  QString modelname = projects->value(pid)->getEPLSModel(mid)->getName();
  ...
  for (size_t j = 0; j < nclass; j++) {
    ...
    NewMatrix(&mx, projects->value(pid)->getEPLSModel(mid)->y_recalculated->row,
              3);
    for (size_t i = 0;
         i < projects->value(pid)->getEPLSModel(mid)->y_recalculated->row;
         i++) {
      mx->data[i][0] =
          projects->value(pid)
              ->getEPLSModel(mid)
              ->y_recalculated->data[i][j + (nclass * nlv) - nclass];
      if (classes[j].objects.contains(objsel[i]) == true) {
        // True
        mx->data[i][1] = 1.f;
      } else {
    ...
    for (int i = 0; i < (int)mx->row; i++) {
      ...
      name += QString("Probability: %1")
                  .arg(QString::number(mx->data[i][0], 'g', 3));
      row_name << name;
    ...
    */
    ...
    for (uint i = 0; i < nobjects; i++) {
      ...
      for (uint j = 0; j < ny; j++) {
        setMatrixValue(
            model_pred_y, i, j,
            getMatrixValue(projects->value(pid)->getEPLSModel(mid)->y_predicted,
                           i, j + (ny * nlv) - ny));
      }
    }
    ...
    for (uint i = 0; i < nobjects; i++) {
      ...
      for (uint j = 0; j < ny; j++) {
        setMatrixValue(
            pred_res, i, j,
            getMatrixValue(
                projects->value(pid)->getEPLSModel(mid)->y_predicted_residuals,
                i, j + (ny * nlv) - ny));
      }
    }
...
QList<SimpleLine2DPlot *> EPLSPlot::ROCAUCs() {
  ...
  QString projectname = projects->value(pid)->getProjectName();
  QString modelname = projects->value(pid)->getEPLSModel(mid)->getName();
  uint nlv = projects->value(pid)->getEPLSModel(mid)->getNPC() +
  ...
  for (uint j = 0; j < yval; j++) {
    QString yname =
        projects->value(pid)->getEPLSModel(mid)->getClasses()[j].name;
    ...
    for (uint i = 0; i < nlv - 1; i++) {
      setMatrixValue(
          m, i + 1, 1,
          getMatrixValue(
              projects->value(pid)->getEPLSModel(mid)->roc_auc_recalculated, i,
              l));
      setMatrixValue(
          m, i + 1, 2,
          getMatrixValue(
              projects->value(pid)->getEPLSModel(mid)->roc_auc_predicted, i,
              l));
    }
    l++;
...
QList<SimpleLine2DPlot *> EPLSPlot::ROCCurves() {
  ...
  uint yval =
      projects->value(pid)->getEPLSModel(mid)->roc_recalculated->m[0]->col / 2.;

  QStringList curvenames;
  curvenames << "ROC Recalculated"
             << "ROC Predicted";

  tensor *roc_recalculated =
      projects->value(pid)->getEPLSModel(mid)->roc_recalculated;
  tensor *roc_predicted =
      projects->value(pid)->getEPLSModel(mid)->roc_predicted;
  ...
  for (uint i = 0; i < nlv; i++) {
    for (uint j = 0; j < yval; j++) {
      ...
      plots.append(new SimpleLine2DPlot(
          mlst, curvenames,
          QString(
              "N. LV: %1 %2 - %3 - ROC Curve Recalculated/Predicted Plot %4")
...
QList<SimpleLine2DPlot *> EPLSPlot::PrecisionRecallAveragePrecision() {
  ...
  for (uint j = 0; j < yval; j++) {
    ...
    plots.append(new SimpleLine2DPlot(
        m, curvenames,
        QString(
            " %1 - %2 - Precision-Recall AUC Recalculated/Predicted Plot %3")
            .arg(projectname)
            .arg(modelname)
            .arg(yname),
...
QList<SimpleLine2DPlot *> EPLSPlot::PrecisionRecallCurves() {
  ...
  uint yval = projects->value(pid)
                  ->getEPLSModel(mid)
                  ->precision_recall_recalculated->m[0]
                  ->col /
              2.;

  QStringList curvenames;
  curvenames << "Precision-Recall Recalculated"
             << "Precision-Recall Predicted";

  tensor *pr_recalculated =
      projects->value(pid)->getEPLSModel(mid)->precision_recall_recalculated;
  tensor *pr_predicted =
      projects->value(pid)->getEPLSModel(mid)->precision_recall_predicted;
  ...
  for (uint i = 0; i < nlv; i++) {
    for (uint j = 0; j < yval; j++) {
      ...
      mlst.append(mpred);
      plots.append(
          new SimpleLine2DPlot(mlst, curvenames,
                               QString("N. LV: %1 %2 - %3 - Precision-Recall "
                                       "Recalculated/Predicted Plot %4")
                                   .arg(QString::number(i + 1))
                                   .arg(projectname)
                                   .arg(modelname)
                                   .arg(yname),
...
QList<SimpleLine2DPlot *> EPLSPlot::R2Q2() {
  ...
  uint nlv = projects->value(pid)->getEPLSModel(mid)->getNPC() +
             1; // +1 because we start from 0
  auto yval = projects->value(pid)->getEPLSModel(mid)->r2->col;
  matrix *m;
  ...
  for (uint j = 0; j < yval; j++) {
    ...
    for (uint i = 0; i < nlv - 1; i++) {
      setMatrixValue(
          m, i + 1, 1,
          getMatrixValue(projects->value(pid)->getEPLSModel(mid)->r2, i, l));
      double q2 =
          getMatrixValue(projects->value(pid)->getEPLSModel(mid)->q2, i, l);
      if (q2 < 0) {
...
QList<SimpleLine2DPlot *> EPLSPlot::R2R2Prediction() {
  ...
  for (uint j = 0; j < yval; j++) {
    ...
    for (uint i = 0; i < nlv - 1; i++) {
      ...
      double r2pred = getMatrixValue(projects->value(pid)
                                         ->getEPLSModel(mid)
                                         ->getEPLSPrediction(predid)
                                         ->r2,
                                     i, l);
...
/*QList<ScatterPlot*> EPLSPlot::YScramblingPlot()
{
  ...
  size_t nobj =
projects->value(pid)->getEPLSModel(mid)->Model()->yscrambling->row; size_t
ndepvar =
(projects->value(pid)->getEPLSModel(mid)->Model()->yscrambling->col-1)/2;
  ...
  for(size_t i = 0; i < nobj; i++){
    mxlst[0]->data[i][0] =
projects->value(pid)->getEPLSModel(mid)->Model()->yscrambling->data[i][0];
    mxlst[1]->data[i][0] =
projects->value(pid)->getEPLSModel(mid)->Model()->yscrambling->data[i][0];
  ...
  for(size_t j = 0; j < ndepvar; j++){
    for(size_t i = 0; i < nobj; i++){
      mxlst[0]->data[i][1] =
projects->value(pid)->getEPLSModel(mid)->Model()->yscrambling->data[i][j+1];
      mxlst[1]->data[i][1] =
projects->value(pid)->getEPLSModel(mid)->Model()->yscrambling->data[i][j+1+ndepvar];
    }
    plots2D.append(new ScatterPlot(mxlst, objnamelst, "Correlation with Y Real
Vector", "R2/Q2", QString("PLS Y Scrambling Plot")));
