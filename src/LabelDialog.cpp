...
{
//   qDebug() << "Show " << pid << " " << lid << " " << lidlst.size();
...
void LabelDialog::Cancel()
{
  reject();
}

void LabelDialog::Close()
{
  accept();
}
...
LabelDialog::LabelDialog(PROJECTS *projects_, int windowtype_): QDialog()
{
  ...
  connect(ui.projectView->selectionModel(), SIGNAL(currentChanged(QModelIndex,QModelIndex)),
          SLOT(GenLabelView(QModelIndex)));
  connect(ui.labelView->selectionModel(), SIGNAL(currentChanged(QModelIndex,QModelIndex)), SLOT(setLabelId(QModelIndex)));
}

LabelDialog::LabelDialog(LABELS * labels_, QStringList& selectedobject_, int windowtype_): QDialog()
{
  ui.setupUi(this);
