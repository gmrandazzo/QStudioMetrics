 ...
 *
 * Implementation file for ClusterModel.
 *
...
void ClusterModel::ImportClusterModel(QString path, QString name_) {
  name = name_;
  string sep = " \t";
  DATAIO::ImportUIvector(QString(path + "/clusters.txt").toUtf8().data(),
                         clusters);
  DATAIO::ImportMatrix(QString(path + "/centroids.txt").toUtf8().data(), sep,
                       centroids);
  DATAIO::ImportMatrix(QString(path + "/validation.txt").toUtf8().data(), sep,
                       validation);
...
void ClusterModel::WriteClusterModel(QString path) {
  ...
  DATAIO::WriteUIvector(QString(path + "/clusters.txt").toUtf8().data(),
                        getClustersVector());
  DATAIO::WriteMatrix(QString(path + "/centroids.txt").toUtf8().data(),
                      getCentroids());
  DATAIO::WriteMatrix(QString(path + "/validation.txt").toUtf8().data(),
                      getValidation());

  QStringList info;
  info.append(QString("NUMBER OF CLUSTERS\t%1")
                  .arg(QString::number(getNumberOfClusters())));
  info.append(QString("INPUT MATRIX HASH\t%1").arg(getHash()));
  info.append(
      QString("CLUSTERING ALGORITHM\t%1").arg(QString::number(clusteralgo)));
