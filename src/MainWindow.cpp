...
void MainWindow::removeModel() {
  if (CurrentIsModel() == true) {
    ...
    if (removemodel == true) {
      ...
      } else if (ui.treeWidget->currentItem()->text(8).compare("EPLS Model") ==
                 0) {
        updateLog(
            QString("Deleting EPLS Model %1\n").arg(getCurrentModelName()));
...
void MainWindow::showEPLSPrediction() {
  if (CurrentIsPrediction() == true) {
    ...
    if (pid > -1 && mid > -1 && tabid > -1) {
      ...
      int predid = getCurrentPredictionID();
      QString tabname = projectname + " - " + modelname +
                        " - EPLS Predicted Dipendent Value" + " - " +
                        ui.treeWidget->currentItem()->text(0);
      MDIChild *child = createMdiChild();
      child->setWindowID(tabid);
      child->newTable(tabname,
      ...
      for (int i = 0; i < projects->value(pid)->getEPLSModel(mid)->getNPC();
           i++) {
        for (size_t j = 0;
             j < projects->value(pid)->getEPLSModel(mid)->Model()->ny; j++) {
  ...
  if (CurrentIsModel() == true) {
    ...
    if (pid > -1 && mid > -1 && tabid > -1) {
      ...
      size_t row = projects->value(pid)
                       ->getEPLSModel(mid)
                       ->Model()
                       ->nlv; // the number of components
      size_t col = 0;
      if (projects->value(pid)->getEPLSModel(mid)->getAlgorithm() == EPLS_)
        col = projects->value(pid)->getEPLSModel(mid)->r2->col +
              projects->value(pid)->getEPLSModel(mid)->q2->col +
              projects->value(pid)->getEPLSModel(mid)->sdep->col +
              projects->value(pid)->getEPLSModel(mid)->bias->col;
      else // eplsda
        col =
            projects->value(pid)->getEPLSModel(mid)->roc_auc_recalculated->col +
            projects->value(pid)->getEPLSModel(mid)->roc_auc_predicted->col +
            projects->value(pid)
                ->getEPLSModel(mid)
                ->precision_recall_ap_recalculated->col +
            projects->value(pid)
                ->getEPLSModel(mid)
                ->precision_recall_ap_predicted->col;
      ...
      if (projects->value(pid)->getEPLSModel(mid)->getAlgorithm() == EPLS_) {
        for (size_t i = 0; i < row; i++) { // PC
          ...
          size_t l = 0;
          for (size_t j = 0;
               j < projects->value(pid)->getEPLSModel(mid)->q2->col;
               j++) { // the q2 for each y
            setMatrixValue(
                child->getTable()->model()->Matrix(), i, l,
                getMatrixValue(projects->value(pid)->getEPLSModel(mid)->r2, i,
                               j));
            l++;
            setMatrixValue(
                child->getTable()->model()->Matrix(), i, l,
                getMatrixValue(projects->value(pid)->getEPLSModel(mid)->q2, i,
                               j));
            l++;
            setMatrixValue(
                child->getTable()->model()->Matrix(), i, l,
                getMatrixValue(projects->value(pid)->getEPLSModel(mid)->sdep, i,
                               j));
            l++;
            setMatrixValue(
                child->getTable()->model()->Matrix(), i, l,
                getMatrixValue(projects->value(pid)->getEPLSModel(mid)->bias, i,
                               j));
            l++;
          }
        }

        QStringList yvarname =
            projects->value(pid)->getEPLSModel(mid)->getYVarName();
        for (size_t j = 0; j < projects->value(pid)->getEPLSModel(mid)->r2->col;
             j++) {
          header << QString("R^2(%1)").arg(yvarname[j])
                 << QString("Q^2(%1)").arg(yvarname[j])
                 << QString("SDEP(%1)").arg(yvarname[j])
                 << QString("BIAS(%1)").arg(yvarname[j]);
        }
      } else {
        PrintMatrix(
            projects->value(pid)->getEPLSModel(mid)->roc_auc_recalculated);
        for (size_t i = 0; i < row; i++) { // PC
          labels.append("PC " + QString::number(i + 1));
          size_t l = 0;
          for (size_t j = 0; j < projects->value(pid)
            ...
            setMatrixValue(
                child->getTable()->model()->Matrix(), i, l,
                getMatrixValue(
                    projects->value(pid)->getEPLSModel(mid)->roc_auc_predicted,
                    i, j));
            l++;
            setMatrixValue(
                child->getTable()->model()->Matrix(), i, l,
        ...
        for (size_t j = 0;
             ...
             j++) {
          header
              << QString("Recalculated ROC AUC (%1)").arg(classes[j].name)
              << QString("Predicted ROC AUC (%1)").arg(classes[j].name)
              << QString("Recalculated Prec-Recall AUC (%1)")
                     .arg(classes[j].name)
              << QString("Predicted Prec-Recall AUC (%1)").arg(classes[j].name);
        }
      }
...
void MainWindow::showEPLSValidatedPrediction() {
  if (CurrentIsModel() == true) {
    ...
    if (pid > -1 && mid > -1 && tabid > -1) {
      ...
      QString tabname =
          projectname + " - " + modelname + " - EPLS Model Prediction";
      MDIChild *child = createMdiChild();
      child->setWindowID(tabid);
      child->newTable(tabname,
                      projects->value(pid)->getEPLSModel(mid)->y_predicted,
                      &projects->value(pid)->getObjectLabels(),
                      &projects->value(pid)->getVariableLabels());
      QStringList varname =
          projects->value(pid)->getEPLSModel(mid)->getObjName();
      child->getTable()->model()->setObjNames(varname);
      QStringList header;
      header << firstcol_name;
      for (size_t i = 0;
           i < projects->value(pid)->getEPLSModel(mid)->Model()->nlv; i++) {
        for (size_t j = 0;
             j < projects->value(pid)->getEPLSModel(mid)->Model()->ny; j++) {
          header << QString("y %1 (PC %2)")
                        .arg(QString::number(j + 1))
...
void MainWindow::showEPLSRecalcY() {
  if (CurrentIsModel() == true) {
    ...
    if (pid > -1 && mid > -1 && tabid > -1) {
      ...
      child->setWindowID(tabid);
      child->newTable(tabname,
                      projects->value(pid)->getEPLSModel(mid)->y_recalculated,
                      &projects->value(pid)->getObjectLabels(),
                      &projects->value(pid)->getVariableLabels());
      child->getTable()->model()->setObjNames(
          projects->value(pid)->getEPLSModel(mid)->getObjName());
  ...
  if (CurrentIsModel() == true) {
    ...
    if (pid > -1 && mid > -1 && tabid > -1) {
      ...
      if (getCurrentModelType().compare("PCA Model") == 0) {
        ...
        validationtype =
            projects->value(pid)->getEPLSModel(mid)->getValidation();
        combinationrule =
            projects->value(pid)->getEPLSModel(mid)->getCombinationRule();
    ...
    if (CurrentIsData() == true) { // this is a Data
      ...
      if (modeltype.compare("CPCA Model") == 0) {
        ...
        if (projects->value(pid)->getEPLSModel(mid)->getValidation() > 0) {
          ...
          menu.addAction("&Show Predicted Y", this,
                         SLOT(showEPLSValidatedPrediction()));
...
void MainWindow::GetEPLSProjects(ProjectTree *pjtree) {
  ...
  for (i = 0; i < projects->keys().size(); i++) {
    ...
    for (j = 0; j < projects->value(pid)->EPLSCount(); j++) {
      ...
      for (k = 0;
           k < projects->value(pid)->getEPLSModelAt(j)->EPLSPredictionCount();
           k++) {
        m.ptree.append(PREDICTIONTREE());
        m.ptree.last().name = projects->value(pid)
                                  ->getEPLSModelAt(j)
    ...
    if (dp.exec() == QDialog::Accepted) {
      ...
      if (projects->value(dp.getProjectID())
        ...
        for (int i = 0; i < cellnames.size(); i++) {
          MDIChild *child = createMdiChild();
          QString tabname =
              QString("EPLS %1 - Class Recalculated vs Experimental")
                  .arg(projects->value(dp.getProjectID())
                           ->getEPLSModel(dp.getModelID())
                           ->getName());
          child->newTable(tabname, cellnames[i], images[i], colors[i]);
  ...
  if (ProjectsHaveEPLSValidated() == true) {
    ...
    if (dp.exec() == QDialog::Accepted) {
      ...
      if (projects->value(dp.getProjectID())
        ...
        for (int i = 0; i < cellnames.size(); i++) {
          MDIChild *child = createMdiChild();
          QString tabname = QString("EPLS %1 - Class Predicted vs Experimental")
                                .arg(projects->value(dp.getProjectID())
                                         ->getEPLSModel(dp.getModelID())
                                         ->getName());
          child->newTable(tabname, cellnames[i], images[i], colors[i]);
...
void MainWindow::EPLSPlotPRAucs() {
  if (ProjectsHaveEPLSValidated() == true) {
    ...
    if (dp.exec() == QDialog::Accepted) {
      ...
      eplsplot.setNLatentVariables(dp.getNLV());
      QList<SimpleLine2DPlot *> plots =
          eplsplot.PrecisionRecallAveragePrecision();
      for (int i = 0; i < plots.size(); i++) {
        MDIChild *graphchild = createMdiChild();
        graphchild->setWidget(plots[i]);
        graphchild->setWindowID(
...
void MainWindow::DoEPLSPrediction() {
  if (ProjectsHaveEPLSValidated() == true) {
    ...
    if (p.exec() == QDialog::Accepted && p.compute() == true) {
      ...
      QStringList xvarsel =
          projects->value(pid)->getEPLSModel(mid)->getXVarName();
      matrix *x, *y;
      initMatrix(&x);
      initMatrix(&y);

      PrepareMatrix(projects->value(pid)->getMatrix(did), objsel, xvarsel, ysel,
                    x, y);
      ...
      if (x->col == (size_t)xvarsel.size()) {
        ...
        projects->value(pid)
            ->getEPLSModel(mid)
            ->getLastEPLSPrediction()
            ->setName("EPLS Prediction - " + modelname);
        projects->value(pid)
            ->getEPLSModel(mid)
            ->getLastEPLSPrediction()
            ->setPredID(
                projects->value(pid)->getEPLSModel(mid)->EPLSPredictionCount() -
                1);
        projects->value(pid)
            ->getEPLSModel(mid)
            ->getLastEPLSPrediction()
        ...
        obj.setElearningParm(
            projects->value(pid)->getEPLSModel(mid)->getElearningParm());

        QFuture<void> future = obj.RunEPLSPrediction(
            projects->value(pid)->getEPLSModel(mid)->getCombinationRule());
        while (!future.isFinished())
          QApplication::processEvents();

        //         ModelPrediction Name - Tab Count - pid - Model ID - xdata id
        //         - ydata id - Data Position - Data Type (PCA Prediction, PLS
        //         Prediction, ...) (8)
        QTreeWidgetItem *subitem = new QTreeWidgetItem;
        subitem->setText(0, projects->value(pid)
                                ->getEPLSModel(mid)
        ...
        // << subitem->text(3) << subitem->text(4) << subitem->text(5); #endif
        ...
        QMessageBox::critical(
            this, tr("EPLS Prediction Error"),
            tr("Unable to compute EPLS Prediction.\n"
               "The number of variables differ. Please check your data."),
            QMessageBox::Ok);
        updateLog(
            QString("Error!! Unable to compute EPLS Prediction. The number of "
                    "variables differ. Please check your data.\n"));
      }
      TopMenuEnableDisable();
      CalculationMenuEnable();
...
void MainWindow::DoEPLSValidation() {
  ...
  if (doeplsval.exec() == QDialog::Accepted && doeplsval.compute() == true) {
    ...
    if (did != -1 && did < projects->value(pid)->MatrixCount()) {
      ...
      CalculationMenuDisable(pid);
      updateLog(
          QString("--------------------\n Computing EPLS Validation for: %1")
      ...
      QFuture<void> future = obj.RunEPLSValidation(
          projects->value(pid)->getEPLSModel(mid)->getAlgorithm(),
          projects->value(pid)->getEPLSModel(mid)->getCombinationRule());
      while (!future.isFinished()) {
        if (stoprun == true) {
          obj.AbortRun();
      ...
      QMessageBox::critical(
          this, tr("EPLS Validation Error"),
          "Unable to compute EPLS Validation.\nData are lost.",
          QMessageBox::Ok);
...
void MainWindow::DoEPLSDA() { DoEPLS(EPLS_DA_); }

void MainWindow::DoEPLSRegression() { DoEPLS(EPLS_); }
  ...
  if (!projects->isEmpty()) {
    ...
    if (doepls.exec() == QDialog::Accepted && doepls.compute() == true) {
      ...
      if (did != -1 && pid != -1 && objsel.size() > 0 && xvarsel.size() > 0 &&
        ...
        projects->value(pid)->getLastEPLSModel()->setDataHash(
            projects->value(pid)->getMatrix(did)->getHash());
        ...
        StopRun();
                 ...
                 xvarsel.size() > 0 && classes.size() > 0) { // EPLSDA
        StartRun();
        CalculationMenuDisable(pid);
        QString str =
...
MainWindow::MainWindow(QString confdir_, QString key_) : QMainWindow(0) {
  ...
  connect(ui.actionEPLS_Regression, SIGNAL(triggered(bool)),
          SLOT(DoEPLSRegression()));
  connect(ui.actionEPLS_Discriminant_Analysis, SIGNAL(triggered(bool)),
          SLOT(DoEPLSDA()));
  connect(ui.actionEPLS_Prediction, SIGNAL(triggered(bool)),
          SLOT(DoEPLSPrediction()));
  connect(ui.actionEPLS_Validator, SIGNAL(triggered(bool)),
          SLOT(DoEPLSValidation()));
  ...
  connect(ui.actionEPLSRecalculated_vs_Experimental, SIGNAL(triggered(bool)),
          SLOT(EPLSRecalcVSExpPlot()));
  connect(ui.actionEPLSPredicted_vs_Experimental, SIGNAL(triggered(bool)),
          SLOT(EPLSPredVSExpPlot()));
  connect(ui.actionEPLSR2_and_Q2, SIGNAL(triggered(bool)),
          SLOT(EPLSPlotR2Q2()));
  connect(ui.actionEPLSROC_curves, SIGNAL(triggered(bool)),
          SLOT(EPLSPlotROCCurves()));
  connect(ui.actionEPLSROC_AUC_recalculated_predicted, SIGNAL(triggered(bool)),
          SLOT(EPLSPlotROCAucs()));
  connect(ui.actionEPLSPrecision_recall_curves, SIGNAL(triggered(bool)),
          SLOT(EPLSPlotPRCurves()));
  connect(ui.actionEPLSPrecision_Recall_AUC_recalculated_predicted,
          SIGNAL(triggered(bool)), SLOT(EPLSPlotPRAucs()));
