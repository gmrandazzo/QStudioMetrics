...
QColor ScatterPlot::makeColor(double val, double min, double max,
...
void ScatterPlot::BuildHotellingEllipse() {
  ...
  rot_mx->data[0][0] = cos(phi);
  rot_mx->data[0][1] = sin(phi);
  rot_mx->data[1][0] = -sin(phi);
  rot_mx->data[1][1] = cos(phi);
  ...
  for (int i = 0; i < chart->getCurves().size(); i++) {
    if (chart->getCurves()[i].name().compare(
            "Hotelling ellipse 95% confidence") == 0) {
      cid = i;
      break;
    } else {
      continue;
  ...
  if (cid > -1) {
    for (size_t i = 0; i < r_ellipse->row; i++) {
      chart->getCurves()[cid].getPoints()[i].setX(r_ellipse->data[i][0] +
                                                  cc->data[0]);
      chart->getCurves()[cid].getPoints()[i].setY(r_ellipse->data[i][1] +
                                                  cc->data[1]);
    }
    chart->getCurves()[cid].getPoints()[r_ellipse->row].setX(
        r_ellipse->data[0][0] + cc->data[0]);
    chart->getCurves()[cid].getPoints()[r_ellipse->row].setY(
        r_ellipse->data[0][1] + cc->data[1]);
  } else {
    QVector<QPointF> curve;
    for (size_t i = 0; i < r_ellipse->row; i++) {
      curve.append(QPointF(r_ellipse->data[i][0] + cc->data[0],
                           r_ellipse->data[i][1] + cc->data[1]));
    }
    curve.append(
  ...
  if (autonameaxes == true) {
    if (axistype == SINGLEAXIS) {
      if (nameaxisext.size() > 0) {
        chart->setXaxisName(QString("%1 %2").arg(xaxisname).arg(
            nameaxisext[ui.axis3->value() - 1]));
        chart->setYaxisName(QString("%1 %2").arg(yaxisname).arg(
            nameaxisext[ui.axis3->value() - 1]));
      } else {
        chart->setXaxisName(QString("%1 %2").arg(xaxisname).arg(
            QString::number(ui.axis3->value())));
        chart->setYaxisName(QString("%1 %2").arg(yaxisname).arg(
            QString::number(ui.axis3->value())));
      }
      ...
      if (nameaxisext.size() > 0) {
        chart->setXaxisName(QString("%1 %2").arg(xaxisname).arg(
            nameaxisext[ui.axis1->value() - 1]));
        chart->setYaxisName(QString("%1 %2").arg(yaxisname).arg(
            nameaxisext[ui.axis2->value() - 1]));
      } else {
        chart->setXaxisName(QString("%1 %2").arg(xaxisname).arg(
            QString::number(ui.axis1->value())));
        chart->setYaxisName(QString("%1 %2").arg(yaxisname).arg(
            QString::number(ui.axis2->value())));
      }
    } else {
      if (nameaxisext.size() > 0) {
        chart->setXaxisName(QString("%1 %2").arg(xaxisname).arg(
        ...
        chart->setYaxisName(QString("%1 %2").arg(yaxisname).arg(
            nameaxisext[ui.axis2->value() - 1]));
        chart->setZaxisName(QString("%1 %2").arg(yaxisname).arg(
            nameaxisext[ui.axis3->value() - 1]));
      } else {
        chart->setXaxisName(QString("%1 %2").arg(xaxisname).arg(
            QString::number(ui.axis1->value())));
...
void ScatterPlot::initPoint(QList<matrix *> mx, QList<matrix *> my,
  ...
  for (int i = 0; i < p.size(); i++) {
    chart->addPoint(p[i].coord[0], p[i].coord[1], p[i].name, p[i].color,
                    p[i].radius);
  }
  UpdateAxisName();
}
...
// For 2D and 3D plots!
  ...
  if (axistype == SINGLEAXIS || axistype == DOUBLEAXIS) {
    for (int i = 0; i < p.size(); i++) {
      chart->addPoint(p[i].coord[x], p[i].coord[y], p[i].name, p[i].color,
                      p[i].radius);
    }
  } else {
    for (int i = 0; i < p.size(); i++) {
      chart->addPoint(p[i].coord[x], p[i].coord[y], p[i].coord[z], p[i].name,
                      p[i].color, p[i].radius);
    }
  }
...
void ScatterPlot::SelectAll() { chart->SelectAll(); }
...
void ScatterPlot::SelectBy() {
  ...
  SelectByDialog seldialog(objname, coordinates, mxlst, mxid, objlabels,
                           varlabels);
...
void ScatterPlot::SelectByVarlabels() {
  ...
  VariableSelectorDialog vseldialog(objnames, varnames, (*mxlst), xid,
                                    varlabels);
        ...
        if (changecolor == SelectionStyleDialog::FIXED) {
          ...
          for (int i = 0; i < selectedIDS.size(); i++) {
            chart->getPoint(selectedIDS[i])
                ->setColor(obj.getSolidSymbolColor());
          }
...
void ScatterPlot::SaveSelection() {
  QString fileName = QFileDialog::getSaveFileName(
      this, tr("Save Selection to..."), "", tr("TXT (*.txt);;All Files (*)"));
...
void ScatterPlot::SavePlotImage() {
  QString fileName = QFileDialog::getSaveFileName(
      ...
      tr("JPEG (*.jpg);;PNG (*.png);; PDF (*.pdf);;All Files (*)"));
...
void ScatterPlot::DoClusterAnalysis() {
  DoClusterAnalysisDialog docluster(mxlst, NULL,
                                    DoClusterAnalysisDialog::PlotSelection);
  if (docluster.exec() == QDialog::Accepted) {
    ...
    if (docluster.getDataType() == 0) {
      ...
      if (dataid > -1) {
        ...
        QMessageBox::warning(this, tr("Clustering Error"),
                             tr("Unable to run clustering\n Please select the "
                                "data where to run the clustering algorithm."),
                             QMessageBox::Ok);
      }
    }
...
void ScatterPlot::CloseFindCorrelationWidget() { cwidget = 0; }
...
void ScatterPlot::FindCorrelations() {
  ...
  cwidget = new FindCorrelationWidget(correl, disterr, &varnames, varlabels,
                                      pid, mid, mtype);

  connect(cwidget, SIGNAL(CloseFindCorrelationWidgetSignal()),
          SLOT(CloseFindCorrelationWidget()));

  if (pid > -1 && mid > -1)
    connect(cwidget, SIGNAL(VVPlotSignal(vvplotSignal)),
            SLOT(EmitVVPlotSignal(vvplotSignal)));

  connect(cwidget, SIGNAL(HighlightSignal(highlightSignal)),
          SLOT(HighlightObjects(highlightSignal)));
...
void ScatterPlot::ShowContextMenu(const QPoint &pos) {
  ...
  if (type == SCORES) {
    ...
    menu.addAction("&Show selected point labels", this,
                   SLOT(showPointLabels()));
    menu.addAction("&Hide selected point labels", this,
                   SLOT(hidePointLabels()));
    /*}*/
    ...
    if (objlabels != 0) {
      menu.addAction("&Set Selection Label", this,
                     SLOT(SetSelectionObjLabels()));
    }
    ...
    if (ehotel == true)
      menu.addAction("&Set OFF Hotelling Confidence Ellipse", this,
                     SLOT(setOnOffHotellingEllipse()));
    else
      menu.addAction("&Set ON Hotelling Confidence Ellipse", this,
                     SLOT(setOnOffHotellingEllipse()));
    ...
    if (varlabels != 0) {
      menu.addAction("&Add Variable Label to Selected", this,
                     SLOT(SetSelectionVarLabels()));
    }
...
void ScatterPlot::slotExit() { qApp->exit(); }
...
ScatterPlot::ScatterPlot(QList<matrix *> &m_, QList<QStringList> &objname,
  ...
  connect(chart->weview(), SIGNAL(customContextMenuRequested(const QPoint &)),
...
ScatterPlot::ScatterPlot(QList<matrix *> &m_, QList<QStringList> &objname,
                         ...
                         QStringList yhash_, LABELS *objlabels_,
                         LABELS *varlabels_, QString xaxsisname_,
                         QString yaxsisname_, QString zaxsisname_,
...
ScatterPlot::ScatterPlot(QList<matrix *> &mx_, QList<matrix *> &my_,
  ...
  if (mx_.size() == my_.size()) {
    ...
    QMessageBox::warning(this, tr("ScatterPlot Error"),
                         tr("Unable to make plot\n Different row/col in mx and "
                            "my. Please check your data."),
                         QMessageBox::Ok);
  }
}
