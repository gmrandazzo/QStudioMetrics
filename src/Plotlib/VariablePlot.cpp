  ...
  if (type == MATRIXDATA) {
    ...
    for (int i = 0; i < objects_.size(); i++) {
      ...
      for (int j = 0; j < xhash.size(); j++) {
        for (uint k = 0;
             k < projects->value(pid)->getMatrix(xhash[j])->Matrix()->row;
             k++) {
          auto indx = objects_[i].indexOf(
              projects->value(pid)->getMatrix(xhash[j])->getObjName()[k]);
          if (indx > -1) {
            nobj++;
          } else {
            continue;
        ...
        for (uint k = 0;
          ...
          if (indx > -1) {
            ...
            setMatrixValue(
                mxlst.last(), c, 0,
                getMatrixValue(
                    projects->value(pid)->getMatrix(xhash[j])->Matrix(), k,
                    varid1));
            setMatrixValue(
                mxlst.last(), c, 1,
                getMatrixValue(
    ...
    for (int i = 0; i < objects_.size(); i++) {
      ...
      for (int j = 0; j < xhash.size(); j++) {
        for (uint k = 0;
             k <
             projects->value(pid)->getArray(xhash[j])->Array()->m[layerid]->row;
             k++) {
          for (int m = 0; m < objects_[i].size(); m++) {
            if (projects->value(pid)
                    ->getArray(xhash[j])
                    ->getObjName()[k]
                    .compare(objects_[i][m]) == 0) {
              QString str = objects_[i][m];
              objects_[i].removeAt(m);
              selobjects.last().append(str);
              setMatrixValue(mxlst.last(), c, 0,
                             getMatrixValue(projects->value(pid)
                                                ->getArray(xhash[j])
                                                ->Array()
                                                ->m[layerid],
                                            k, varid1));
              setMatrixValue(mxlst.last(), c, 1,
                             getMatrixValue(projects->value(pid)
                                                ->getArray(xhash[j])
                                                ->Array()
                                                ->m[layerid],
        ...
        for (uint k = 0;
          ...
          for (int m = 0; m < objects[i].size(); m++) {
            if (projects->value(pid)
              ...
              QString str = objects[i][m];
              objects[i].removeAll(str);
              DVectorAppend(v.last(), getMatrixValue(projects->value(pid)
                                                         ->getArray(xhash[j])
                                                         ->Array()
                                                         ->m[layerid],
                                                     k, varid1));
