...
void help(char **argv) {
  ...
  std::cout << "Make the cross validation: " << argv[0]
            << " -cv -x  <xinput file>  -y <yinput file> -dm <input path "
               "model> -g <N° of groups> -i <N° iterations> -nth <N° threads>\n"
            << std::endl;
  ...
  if (argc <= 1) {
    ...
    if (genmodel == true && !xinputdata.empty() && !yinputdata.empty() &&
      ...
      DelMatrix(&ydata);
               ...
               !xinputdata.empty() && !outputfile.empty()) {
      matrix *xdata;
      initMatrix(&xdata);
      ...
      BootstrapRandomGroupsCV(&minpt, ngroups, iterations, _MLR_, pred,
      ...
      for(size_t i = 0; i < q2y->row; i++){
        for(size_t j = 0; j < q2y->col; j++){
          if(j == 0)
            cout << "PC " << i+1 << ": " << setw(3) << getMatrixValue(q2y, i,
      j); else if(j == q2y->col-1) cout << setw(10) << getMatrixValue(q2y, i,
      j); else cout << setw(10) << getMatrixValue(q2y, i, j);
        }
        cout << endl;
      }
      ...
      for(size_t i = 0; i < sdep->row; i++){
        for(size_t j = 0; j < sdep->col; j++){
          if(j == 0)
            cout << "PC " << i+1 << ": " << setw(3) << getMatrixValue(sdep, i,
      j); else if(j == sdep->col-1) cout << setw(10) << getMatrixValue(sdep, i,
      j); else cout << setw(10) << getMatrixValue(sdep, i, j);
        }
        cout << endl;
      }
      ...
      for(size_t i = 0; i < bias->row; i++){
        for(size_t j = 0; j < bias->col; j++){
          if(j == 0)
            cout << "PC " << i+1 << ": " << setw(3) << getMatrixValue(bias, i,
      j); else if(j == bias->col-1) cout << setw(10) << getMatrixValue(bias, i,
      j); else cout << setw(10) << getMatrixValue(bias, i, j);
        }
        cout << endl;
      }
      ...
      for(size_t i = 0; i < q2y_consistency->row; i++){
        for(size_t j = 0; j < q2y_consistency->col; j++){
          if(j == 0)
            cout << "PC " << i+1 << ": " << setw(3) <<
      getMatrixValue(q2y_consistency, i, j); else if(j ==
      q2y_consistency->col-1) cout << setw(10) <<
      getMatrixValue(q2y_consistency, i, j) << endl; else cout << setw(10) <<
      ...
      for(size_t i = 0; i < sdep_consistency->row; i++){
        for(size_t j = 0; j < sdep_consistency->col; j++){
          if(j == 0)
            cout << "PC " << i+1 << ": " << setw(3) <<
      getMatrixValue(sdep_consistency, i, j); else if(j ==
      sdep_consistency->col-1) cout << setw(10) <<
      getMatrixValue(sdep_consistency, i, j) << endl; else cout << setw(10) <<
