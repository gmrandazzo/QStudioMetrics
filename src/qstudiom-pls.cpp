 ...
 *
 * Implementation file for qstudiom-pls.
 *
...
void help(char **argv) {
  ...
  std::cout << "Make a model: " << argv[0]
            << " -(r/d)model -x <xinput file>  -y <yinput file>"
            << " -o <output file> -c <N° of LV> -xa(autoscaling for x matrix) "
               "-ya(autoscaling for y matrix)\n"
            << std::endl;
  std::cout << "Make a prediction: " << argv[0]
            << " -predict -dm <input model> -x <xinput file> -o <output file> "
               "-c <N° of LV>\n"
            << std::endl;
  std::cout
      << "Make the cross validation: " << argv[0]
      << " -cv -x  <xinput file>  -y <yinput file> -dm <input path model> -g "
         "<N° of groups> -i <N° iterations> -c <N° of LV> -nth <N° threads>\n"
      << std::endl;

  std::cout
      << "Calculate beta coefficients: " << argv[0]
      << " -betas -x  <xinput file>  -y <yinput file> -dm <input path model>  "
         "-c <N° of LV> -xa [0, 1, 2, 3, 4, 5] -ya [0, 1, 2, 3, 4, 5]\n"
      << std::endl;
...
int main(int argc, char **argv) {
  if (argc <= 1) {
    ...
    r_genmodel = d_genmodel = genbetas = makeprediction = makecrossvalidation =
        false;
    xsep = ysep = ";";
    ...
    if ((r_genmodel == true || d_genmodel == true) && !xinputdata.empty() &&
        !yinputdata.empty() && !outputfile.empty() && npc > 0) {
      matrix *xdata, *ydata;
      ...
      if (r_genmodel == true)
        PLSRegressionStatistics(ydata, m->recalculated_y, m->r2y_recalculated,
                                m->sdec, NULL);
      else
      ...
      DelMatrix(&xdata);
      DelMatrix(&ydata);
    } else if (genbetas == true && !xinputdata.empty() && !yinputdata.empty() &&
               npc > 0) {
      matrix *xdata, *ydata;
      dvector *betas;
      initMatrix(&xdata);
      ...
      DelMatrix(&ydata);
               ...
               !xinputdata.empty() && !outputfile.empty() && npc > 0) {
      matrix *xdata;
      initMatrix(&xdata);
      ...
      DelMatrix(&xdata);
               ...
               !yinputdata.empty() && ngroups > 0 && npc > 0) {
      matrix *xdata, *ydata;
      ...
      BootstrapRandomGroupsCV(&minpt, ngroups, iterations, _PLS_, pred,
                              predresiduals, nthreads, NULL, 0);
      //      LeaveOneOut(&minpt, _PLS_, &m->predicted_y, &m->pred_residuals, 4,
      //      NULL, 0);
      ...
      for (size_t i = 0; i < q2y_consistency->row; i++) {
        for (size_t j = 0; j < q2y_consistency->col; j++) {
          if (j == 0)
            cout << "PC " << i + 1 << ": " << setw(3)
                 << getMatrixValue(q2y_consistency, i, j);
      ...
      for (size_t i = 0; i < sdep_consistency->row; i++) {
        for (size_t j = 0; j < sdep_consistency->col; j++) {
          if (j == 0)
            cout << "PC " << i + 1 << ": " << setw(3)
                 << getMatrixValue(sdep_consistency, i, j);
