...
public:
  ...
  void delCPCAPredictionAt(int id) {
  ...
  CPCAPREDICTION *getCPCAPrediction(int id) {
  ...
  QString &getHash() {
    if (cpcahash.size() == 0) {
      LABEL v;
      foreach (v, varname)
        cpcahash = GenHashFromStrlst(
            (QStringList() << name << "cpcamodel_type") + objname + v.objects);
    }
    return cpcahash;
  }
